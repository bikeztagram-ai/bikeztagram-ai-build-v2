...
const selected = selectDirectorMoments(moments, {
  maxCuts: 5,
  targetDuration: 15,
  creativePrompt: 'dark cinematic motorcycle reveal with action',
});
...
assert.ok(selected.some((m) => m.directorStoryRole === 'reveal'));
assert.ok(selected.some((m) => m.directorStoryRole === 'action'));
assert.ok(selected.every((m, i) => m.directorStoryPosition === i));
console.log('director-story-order: PASS');
