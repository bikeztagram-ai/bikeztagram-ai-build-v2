...
if((render.timeline.closedGaps||0)<=0)throw new Error('Timeline did not report closed editorial gaps.');
if(render.timeline.cuts.some((c,i)=>i>0&&Math.abs(c.startTime-(render.timeline.cuts[i-1].startTime+render.timeline.cuts[i-1].duration))>.001))throw new Error('Normalized cuts are not contiguous.');
...
console.log('PASS: renderer contract closes editorial gaps and preserves long-form audio continuity.');
