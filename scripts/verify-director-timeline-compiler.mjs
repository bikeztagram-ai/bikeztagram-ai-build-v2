...
const productionPlan = {
  ...
  scenes: [
    ...
    { sourceType: 'uploaded', startTime: 12, duration: 2.5, purpose: 'real-hero-ending', motionStyle: 'slow-pull', motionIntensity: 0.5, speed: 0.9, speedEnd: 0.82 }
  ]
};

const timeline = compileDirectorTimeline(productionPlan, { bpm: 120 });
assert.equal(timeline.cuts.length, 4);
assert.equal(timeline.cuts[0].transition, 'fade-in');
assert.equal(timeline.cuts[3].transition, 'fade-out');
assert.ok(timeline.cuts.some((cut) => cut.transition === 'flash-cut'));
