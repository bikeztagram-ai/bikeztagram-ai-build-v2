...
assert(!exists('api/private-blob-read.js'), 'Obsolete private Blob reader must not be present in this signed-URL pipeline.');
...
if (exists('api/blob-presign.js')) {
  ...
  assert(presign.includes('presignedUrl: readUrl') || presign.includes('url: readUrl'), 'Blob presign endpoint no longer returns a signed read URL.');
...
for (const file of ['api/analyse.js','api/analyse-image.js','api/captions.js']) {
  if (exists(file)) {
    ...
    assert(source.includes('fetch('), `${file} no longer reads the signed Blob URL directly.`);
  }
}
if (exists('api/analyse-library.js')) {
  ...
  assert(library.includes('fetch(url)'), 'Mixed-media Gemini analysis does not read the signed Blob URL directly.');
...
console.log('Signed Blob upload/read, Gemini analysis, local original audio, renderer/QA and final audio mux contracts are present.');
