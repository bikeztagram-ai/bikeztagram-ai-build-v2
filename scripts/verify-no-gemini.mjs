...
const forbidden = [/@google\/genai/i,/GEMINI_API_KEY/i,/gemini-cli/i,/gemini-resilient/i,/gemini-2\./i];
...
function walk(dir) {
  for (const entry of fs.readdirSync(dir,{withFileTypes:true})) {
    ...
    if (entry.isDirectory()) walk(full);
      ...
      if (forbidden.some((rx) => rx.test(text))) violations.push(path.relative(root,full));
    }
  }
}
