...
for(const [name,source] of Object.entries({analyse,analyseImage,captions})){
 assert.doesNotMatch(source,/private-blob-read\.js/,`${name} must not depend on the deleted private Blob helper`);
}
assert.match(analyseLibrary,/from '@vercel\/blob'/,'Mixed-media analysis must use the Vercel Blob SDK');
assert.match(analyseLibrary,/get\(pathname,\{access:'private',token,useCache:false\}\)/,'Mixed-media analysis must perform an authenticated private Blob SDK read');
assert.match(analyseLibrary,/BLOB_READ_WRITE_TOKEN/,'Mixed-media analysis must authenticate the private Blob read');
assert.match(analyseLibrary,/createWriteStream/,'Mixed-media analysis must stream source media to temporary storage');
assert.match(analyseLibrary,/ai\.files\.upload/,'Mixed-media analysis must upload actual source media to Gemini');
assert.match(analyseLibrary,/generateContent/,'Mixed-media analysis must execute the Gemini director pass');

console.log('Batch 76 signed Blob upload/read contract: PASS');
console.log('- time-bound signed PUT URL');
console.log('- time-bound signed GET URL for browser/media access');
console.log('- authenticated server-side private Blob SDK reads');
console.log('- streamed Gemini ingestion');
