...
if (!apiKey) {
  console.log('AUTONOMOUS IMPROVER: GEMINI_API_KEY is not configured; leaving code unchanged.');
  process.exit(0);
}
if (iteration >= maxIterations) {
  console.log(`AUTONOMOUS IMPROVER: iteration limit ${maxIterations} reached.`);
  process.exit(0);
}
...
if (runId && repo) {
  try {
    const jobs = await fetch(`https://api.github.com/repos/${repo}/actions/runs/${runId}/jobs`, {
    ...
    testContext = `Could not read workflow metadata: ${error.message}`;
  }
}
...
- If the evidence does not justify a change, return an empty changes object.

Current autonomous iteration: ${iteration + 1}/${maxIterations}
Test context:
${testContext}
...
{"summary":"short reason","changes":{"src/file.js":"complete replacement file contents"}}
The changes object may contain zero or one file. Do not return patches or markdown.`;
...
const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`, {
  ...
  body: JSON.stringify({
    ...
    generationConfig: { responseMimeType: 'application/json', responseSchema: schema, temperature: 0.15 }
  })
});
...
if (keys.length === 0) {
  console.log(`AUTONOMOUS IMPROVER: no justified change. ${plan.summary || ''}`);
  process.exit(0);
}
...
try {
  ...
  console.error('AUTONOMOUS IMPROVER: build failed; reverting change.');
  sh('git', ['restore', '--', ...keys]);
  process.exit(1);
}
...
if (!status) {
  console.log('AUTONOMOUS IMPROVER: no effective diff.');
  process.exit(0);
}
sh('git', ['config', 'user.name', 'Bikeztagram Autonomous Improver']);
sh('git', ['config', 'user.email', '41898282+github-actions[bot]@users.noreply.github.com']);
sh('git', ['add', ...keys]);
sh('git', ['commit', '-m', `[auto-improve ${iteration + 1}] ${String(plan.summary || 'guarded improvement').slice(0, 120)}`]);
