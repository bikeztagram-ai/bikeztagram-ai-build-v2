...
assert(productionSpecialists.length===4,'Exactly two verified specialist Builders are authorised by the current gate.');
assert(JSON.stringify(productionSpecialists.map(b=>b.id))===JSON.stringify(['director-builder','timeline-builder','music-builder','scene-builder']),'Production specialist set changed.');
const scopes={
