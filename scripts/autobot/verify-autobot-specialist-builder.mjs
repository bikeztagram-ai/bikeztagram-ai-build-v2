...
assert(runner.includes("npm",'Specialist Builder must invoke npm verification/build tooling.');
...
for(const bot of specialists){
  ...
  assert(JSON.stringify(bot.ownsFiles)===JSON.stringify(expectedScopes[bot.id]),`Specialist Builder ${bot.id} ownsFiles scope changed without updating its contract.`);
...
assert(packageJson.scripts?.['verify:autobot-specialist-builder']==='node scripts/autobot/verify-autobot-specialist-builder.mjs','package.json must expose the exact specialist Builder verifier command.');
