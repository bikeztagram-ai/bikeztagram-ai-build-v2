...
assert(workflow.includes("AUTOBOT_LANE_CYCLE_MINUTES: '10'")&&laneRunner.includes("AUTOBOT_LANE_CYCLE_MINUTES || '20'"),'the persistent lane must use the configured 10-minute internal product-evolution cycle while owning the cumulative deadline; it must not spend the fleet finish-grace reserve inside every short internal cycle.');
