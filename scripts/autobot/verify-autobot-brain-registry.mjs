#!/usr/bin/env node
/** Verify that AutoBot's roadmap, objective registry, task library and active queue agree. */
...
for(const id of roadmapIds)if(!objectiveIds.has(id))failures.push(`roadmap objective ${id} is missing from feature-objectives.json`);
for(const task of tasks.tasks||[])if(task.objectiveId&&!objectiveIds.has(task.objectiveId))failures.push(`task ${task.id} references unknown objective ${task.objectiveId}`);
...
console.log(`AutoBot brain registry PASS: roadmap=${roadmapIds.size} registered=${objectiveIds.size} tasks=${(tasks.tasks||[]).length} batch103=${Boolean(batch103)}`);
