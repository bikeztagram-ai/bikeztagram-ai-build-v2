...
const checks=[
 ...
 [planner.includes('const availableFiles=new Set(Array.isArray(inventoryFiles)?inventoryFiles:[])')&&planner.includes('const invForRnd=availableFiles;')&&planner.indexOf('const invForRnd=new Set(inv);')<planner.indexOf('const rndRecommendations=Array.isArray(rnd?.recommendations)'),'R&D fallback inventory must be initialized before recommendations are inspected; no temporal-dead-zone access is allowed'],
