...
const checks=[
 ...
 [ /const cycleDurationsMs=\[\];/.test(engine)&&/const observedCycleMs=cycleDurationsMs\.length\?/.test(engine)&&/const estimatedCycleMs=observedCycleMs\?/.test(engine)&&/const requiredStartMs=estimatedCycleMs\+safetyMinutes\*60_000;/.test(engine)&&/if\(remainingNormalMs\(\)<requiredStartMs\)/.test(engine)&&/launch budget/.test(engine),'cycle launch gate must adapt to observed verified cycle runtime so recovery can cross the historical 30-minute boundary'],
