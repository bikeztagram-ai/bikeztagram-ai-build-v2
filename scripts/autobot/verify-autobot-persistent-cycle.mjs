...
const checks=[
 ...
 [planner.includes('const invForRnd=invSet;')&&planner.indexOf('const invForRnd=new Set(inv);')<planner.indexOf('const rndRecommendations=Array.isArray(rnd?.recommendations)'),'R&D fallback inventory must be initialized before recommendations are inspected; no temporal-dead-zone access is allowed'],
