...
const checks=[
 ...
 [read('builder/runner/aider-feature-brain.mjs').includes('const specialistEditFormat=String(process.env.AUTOBOT_SPECIALIST_AIDER_EDIT_FORMAT||\'diff\').trim().toLowerCase()')&&read('builder/runner/aider-feature-brain.mjs').includes('const cwd=root;const aiderFiles=files')&&read('builder/runner/aider-feature-brain.mjs').includes('const specialistMapTokens=')&&read('builder/runner/aider-feature-brain.mjs').includes("const specialistMapArg=specialist?'--map-tokens=0'")&&read('builder/runner/aider-feature-brain.mjs').includes("'--architect'")&&read('builder/runner/aider-feature-brain.mjs').includes('`--editor-model=${editorModel}`')&&read('builder/runner/aider-feature-brain.mjs').includes("'--editor-edit-format=editor-diff'")&&read('builder/runner/aider-feature-brain.mjs').includes('--subtree-only')&&read('builder/runner/aider-feature-brain.mjs').includes('--edit-format=${specialistEditFormat}')&&!read('builder/runner/aider-feature-brain.mjs').includes("specialist?['--no-git'") ,'focused specialist Aider runs must use the scoped repository map from the repository root with the configured specialist edit format; specialist editing must not use the Repair Bot no-git mode'],
 ...
 [read('builder/runner/autobot-repair.mjs').includes("'--no-git'")&&read('builder/runner/autobot-repair.mjs').includes("--edit-format=${editFormat}")&&read('builder/runner/aider-feature-brain.mjs').includes('const specialistEditFormat=String(process.env.AUTOBOT_SPECIALIST_AIDER_EDIT_FORMAT||\'diff\').trim().toLowerCase()')&&read('builder/runner/aider-feature-brain.mjs').includes('const specialistMapTokens=')&&read('builder/runner/aider-feature-brain.mjs').includes("const specialistMapArg=specialist?'--map-tokens=0'")&&read('builder/runner/aider-feature-brain.mjs').includes("'--architect'")&&read('builder/runner/aider-feature-brain.mjs').includes('`--editor-model=${editorModel}`')&&read('builder/runner/aider-feature-brain.mjs').includes("'--editor-edit-format=editor-diff'")&&read('builder/runner/aider-feature-brain.mjs').includes('--subtree-only')&&read('builder/runner/aider-feature-brain.mjs').includes('--edit-format=${specialistEditFormat}') ,'Repair Aider sessions must disable Aider git/repo scanning while specialist editing retains scoped repo-map context; the outer AutoBot owns git state, scope, commits and verification'],
