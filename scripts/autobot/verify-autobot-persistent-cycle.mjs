...
const checks=[
 ...
 [read('builder/runner/autobot-fleet-recovery.mjs').includes('open recovery failure')&&read('builder/runner/autobot-fleet-recovery.mjs').includes('openFailures'),'fleet recovery must fail clearly when a requested durable failure id is missing instead of dereferencing an undefined failure'],
  [reviewer.includes('repairBaseCommit:candidate')&&reviewer.includes('candidateCommit:candidate'),'reviewer repair failures must preserve the exact reviewed candidate as the Repair Bot base'],
  [recovery.includes('maxReviewerRecoveryDepth=2')&&recovery.includes('recoveryDepth+1'),'Reviewer-triggered recovery must be bounded and recurse through the same Repair -> QA -> Reviewer chain'],
