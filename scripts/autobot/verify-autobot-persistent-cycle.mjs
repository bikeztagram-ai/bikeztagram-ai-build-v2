...
const checks=[
 ...
 [read('builder/runner/aider-feature-brain.mjs').includes("specialist?['--map-tokens=2048','--subtree-only',`--edit-format=udiff`]")&&read('builder/runner/aider-feature-brain.mjs').includes('const cwd=root;const aiderFiles=files')&&!read('builder/runner/aider-feature-brain.mjs').includes("specialist?['--no-git',`--edit-format=udiff`]") ,'focused specialist Aider runs must use the scoped repository map from the repository root so Aider can edit real files with sufficient contract context'],
 ...
 [read('builder/runner/autobot-repair.mjs').includes("'--no-git'")&&read('builder/runner/autobot-repair.mjs').includes("--edit-format=${editFormat}")&&read('builder/runner/aider-feature-brain.mjs').includes("specialist?['--map-tokens=2048','--subtree-only',`--edit-format=udiff`]") ,'Repair Aider sessions must disable Aider git/repo scanning while specialist editing retains scoped repo-map context; the outer AutoBot owns git state, scope, commits and verification'],
