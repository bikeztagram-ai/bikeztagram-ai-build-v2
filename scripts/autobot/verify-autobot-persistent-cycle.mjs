...
const checks=[
 ...
 [read('builder/runner/aider-feature-brain.mjs').includes('const specialistEditFormat=String(process.env.AUTOBOT_SPECIALIST_AIDER_EDIT_FORMAT||\'diff\').trim().toLowerCase()')&&read('builder/runner/aider-feature-brain.mjs').includes('const cwd=root;const aiderFiles=files')&&read('builder/runner/aider-feature-brain.mjs').includes('const specialistMapTokens=')&&read('builder/runner/aider-feature-brain.mjs').includes('--map-tokens=${specialistMapTokens}')&&read('builder/runner/aider-feature-brain.mjs').includes('--subtree-only')&&read('builder/runner/aider-feature-brain.mjs').includes('--edit-format=${specialistEditFormat}')&&!read('builder/runner/aider-feature-brain.mjs').includes("specialist?['--no-git'") ,'focused specialist Aider runs must use the scoped repository map from the repository root with the configured specialist edit format; specialist editing must not use the Repair Bot no-git mode'],
