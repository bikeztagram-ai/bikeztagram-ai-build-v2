...
const checks=[
 ...
 [read('builder/runner/aider-feature-brain.mjs').includes("const specialistEditFormat=String(process.env.AUTOBOT_SPECIALIST_AIDER_EDIT_FORMAT||'udiff').trim().toLowerCase()")&&read('builder/runner/aider-feature-brain.mjs').includes("const cwd=root;const aiderFiles=files;")&&read('builder/runner/aider-feature-brain.mjs').includes("const specialistMapArg=specialist?`--map-tokens=2048`")&&read('builder/runner/aider-feature-brain.mjs').includes('--subtree-only')&&read('builder/runner/aider-feature-brain.mjs').includes('`--edit-format=${specialistEditFormat}`')&&!read('builder/runner/aider-feature-brain.mjs').includes("specialist?['--no-git'")&&!read('builder/runner/aider-feature-brain.mjs').includes('--model-settings-file'),'specialist Aider runs must use the proven root worktree, 2048-token scoped map, direct edit format and must not use Repair/architect no-git or model-settings mode'],
