...
const checks=[
 ...
 [read('builder/runner/aider-feature-brain.mjs').includes("specialist?['--no-git',`--edit-format=udiff`]")&&read('builder/runner/autobot-repair.mjs').includes('--no-git')&&!read('builder/runner/aider-feature-brain.mjs').includes('`--map-tokens=${specialist?0:768}`'),'focused specialist Aider runs must disable repo-map/git scanning to avoid unnecessary repository context work'],
 ...
 [planner.includes('autobot-rnd-brief.json')&&planner.includes('R&D recommendations')&&planner.includes('R&D research is evidence'),'Planner must consume R&D evidence without treating it as unchecked implementation authority'],
  [planner.includes('rndRecommendations')&&planner.includes('R&D — ${title}')&&planner.includes('fallback(bot,library,inv,completedTitles,new Set(),staleAcceptance,rnd)'),'deterministic planner fallback must consume valid R&D recommendations when AI discovery is unavailable'],
 [planner.includes("AI planner returned no usable packages")&&planner.includes("source='hybrid-ai-deterministic'")&&planner.includes('const replacement=fallback(bot,library,inv,completedTitles,seenTitles,staleAcceptance,rnd)'),'Planner must salvage usable AI packages per specialist and deterministically fill only missing or invalid lanes instead of discarding the entire AI plan'],
