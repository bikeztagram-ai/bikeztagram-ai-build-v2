 ...
 * It replaces Ollama with a scripted /api/chat server and runs the real runtime
 * hardening plus the real brain through failure recovery:
 * read_file -> invalid edit -> syntax rollback -> different-file edit -> verify -> submit.
 */
...
const hardenerSource = fs.readFileSync(path.join(root, 'scripts/autobot/fast-brain-runtime-hardening.mjs'), 'utf8');
const taskSource = fs.readFileSync(path.join(root, 'builder/brain/task-library.json'), 'utf8');
...
try {
  ...
  write('scripts/autobot/fast-brain-runtime-hardening.mjs', hardenerSource);
  write('builder/brain/task-library.json', taskSource);
  ...
  write('package.json', JSON.stringify({ name: 'fast-brain-harness', private: true, type: 'module', scripts: { build: 'node -e "process.exit(0)"' } }, null, 2));
  ...
  console.log('[autobot] Fast Brain agent harness PASS: runtime hardening, real JS/JSX syntax validation, native Ollama request contract, named tool results, transactional rollback, failed-file steering, source editing, verification, submit and durable completion all passed.');
} finally {
