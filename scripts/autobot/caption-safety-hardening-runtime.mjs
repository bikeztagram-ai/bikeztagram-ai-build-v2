...
try{
 ...
 const legacyAnchor='const cues=normaliseSpeechCaptions(captions);';
 const legacyWiring='const cues=filterCaptionCues(normaliseSpeechCaptions(captions),options);';
 const currentWiring='const cues=normaliseCaptionTiming(filterCaptionCues(normaliseSpeechCaptions(captions),options));';
 if(source.includes(currentWiring)||source.includes(legacyWiring)){
  console.log('[autobot] Caption safety helper already wired into planner.');
 }else{
  if(!source.includes(legacyAnchor))throw new Error('caption planner cue anchor not found; refusing blind integration edit.');
  source=source.replace(legacyAnchor,legacyWiring);
