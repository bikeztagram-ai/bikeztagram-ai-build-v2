...
const maxGenerated=Number.parseInt(process.env.AUTOBOT_MAX_GENERATED_WAVES||'3',10);
const wave=(ledger.waves?.length||0)+1;
if(wave>maxGenerated){console.log(`[autobot] generated-wave cap ${maxGenerated} reached; stopping safely`);process.exit(0);}
...
{id:'quality-wave-replenish-next',kind:'self-improvement',files:['builder/brain/roadmap.json','builder/brain/task-library.json','builder/brain/self-improvement-ledger.json'],goal:'Continue the bounded production-quality backlog only while the per-run generated-wave safety cap has not been reached.',implementation:['node scripts/autobot/replenish-production-backlog.mjs'],verify:['node --check scripts/autobot/replenish-production-backlog.mjs']},
