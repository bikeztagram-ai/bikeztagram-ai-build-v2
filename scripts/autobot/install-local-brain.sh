...
validate_model() {
  curl -fsS --max-time 90 http://127.0.0.1:11434/api/chat -H 'Content-Type: application/json' -d "{\"model\":\"$MODEL\",\"stream\":false,\"options\":{\"temperature\":0,\"num_ctx\":4096,\"num_predict\":200},\"messages\":[{\"role\":\"user\",\"content\":\"Reply READY.\"}]}" >/tmp/bikeztagram-ollama-smoke.json
  curl -fsS --max-time 90 http://127.0.0.1:11434/api/chat -H 'Content-Type: application/json' -d "{\"model\":\"$MODEL\",\"stream\":false,\"tools\":[{\"type\":\"function\",\"function\":{\"name\":\"probe\",\"description\":\"A readiness probe.\",\"parameters\":{\"type\":\"object\",\"properties\":{},\"required\":[]}}}],\"options\":{\"temperature\":0,\"num_ctx\":4096,\"num_predict\":200},\"messages\":[{\"role\":\"user\",\"content\":\"Call the probe tool exactly once now.\"}]}" >/tmp/bikeztagram-ollama-tool-smoke.json
