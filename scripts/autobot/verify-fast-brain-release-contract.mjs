 ...
 *
 * This is intentionally a cross-file audit: changing one component of the
 * builder must not silently leave another component on the retired architecture.
 */
...
require(!workflow.includes('feature-brain.mjs'), 'workflow directly invokes the brain instead of the canonical executor');
require(!workflow.match(/qwen2\.5|gemini/i), 'workflow contains a retired/non-local model reference');
...
// Safety/recovery contract must exist in source or be provably installed by the
// migration hardener before Qwen is allowed to run.
...
require(rollback.includes('actual-source fixture'), 'rollback regression is not tied to the active source');
...
// Guard against the stale export acceptance command that previously blocked
...
console.log('[autobot] Fast Brain release architecture contract PASS: one canonical Qwen3 4B path, multi-turn tool agent, transactional recovery, bounded execution, and review-only checkpointing.');
