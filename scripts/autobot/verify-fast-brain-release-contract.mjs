...
// Match the active implementation shape, not a brittle quote-specific spelling.
...
// A stale generated command is permitted only when the explicit migration hardener can repair it before Qwen runs.
...
console.log('[autobot] Fast Brain release architecture contract PASS: canonical Qwen3 4B, multi-turn tool agent, transactional recovery, exact checkout verification, migration hardening, honest success metrics, and review-only checkpointing.');
