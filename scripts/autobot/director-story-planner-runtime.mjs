...
if (!planner.includes('const storyBeats=buildDirectorStory(')) {
  ...
  planner = planner.replace(match[0], `${match[0].replace(/\s*let\s+cuts\s*=\s*\[\];$/, '')};const storyBeats=buildDirectorStory(rawMoments,{creativePrompt:options.creativePrompt,targetDuration});let cuts=[];`);
}
...
if (!planner.includes('storyBeats:storyBeats.map(')) {
  const returnMatch = planner.match(/directorSelection\s*:\s*selectedMoments\.map\(\(m\)=>\(\{sourceIndex:m\.sourceIndex,mediaIndex:m\.mediaIndex,mediaId:m\.mediaId,score:m\.directorSelectionScore\}\)\)/);
  if (!returnMatch) throw new Error('director selection return evidence anchor not found; refusing blind edit.');
  planner = planner.replace(returnMatch[0], `${returnMatch[0]},storyBeats:storyBeats.map((m)=>({mediaIndex:m.mediaIndex,role:m.directorStoryRole,score:m.directorStoryScore}))`);
}
