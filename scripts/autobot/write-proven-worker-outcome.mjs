...
const outcome={schemaVersion:'autobot-proven-worker-outcome-v1',workerId,status,repairable:status==='failure'&&patchExists,category:status==='failure'?(patchExists?'product-change':'infrastructure-or-no-candidate'):'completed',objectiveId:checkpoint?.objectiveId||null,error,baseCommit:fs.existsSync('builder/working/autobot-proven-worker-base-sha.txt')?fs.readFileSync('builder/working/autobot-proven-worker-base-sha.txt','utf8').trim():null,patchPath:patchExists?'builder/working/autobot-proven-worker.patch':null,files,evidence:['builder/working/deterministic-autobot.json','builder/working/deterministic-autobot-evidence.json','builder/working/long-run-state.json']};
