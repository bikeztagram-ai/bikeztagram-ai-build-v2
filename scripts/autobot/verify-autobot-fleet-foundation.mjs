...
assert(Number.isInteger(registry.coordination?.maxConcurrentWorkers)&&registry.coordination.maxConcurrentWorkers>=1&&registry.coordination.maxConcurrentWorkers<=10,'fleet worker limit must remain bounded at eight');
if(live){
 ...
 assert(JSON.stringify(registry.activationGate?.parallelWorkers||[])===JSON.stringify(['director-builder','timeline-builder','music-builder','scene-builder','rhythm-builder','render-builder','media-intelligence-builder','caption-builder','intent-builder','continuity-builder']),'parallel activation must name exactly the ten isolated production specialists');
...
assert(productionSpecialists.length===10,'exactly eight verified specialist Builders are authorised by the current live gate');
assert(JSON.stringify(productionSpecialists.map(b=>b.id))===JSON.stringify(['director-builder','timeline-builder','music-builder','scene-builder','rhythm-builder','render-builder','media-intelligence-builder','caption-builder']),'production specialist activation set changed');
const scopes={'director-builder':['src/director.js'],'timeline-builder':['src/executableTimeline.js'],'music-builder':['src/musicDirector.js'],'scene-builder':['src/universalCreativeSceneEngine.js'],'rhythm-builder':['src/editorialRhythm.js'],'render-builder':['src/cinematicRendererV3.js'],'media-intelligence-builder':['src/aiEditPlanner.js'],'caption-builder':['src/captionPlanner.js'],'experimental-media-whole-root':['src/aiEditPlanner.js'],'experimental-caption-diff-root':['src/captionPlanner.js'],'intent-builder':['src/creativeIntentCompiler.js'],'continuity-builder':['src/creativeContinuityEngine.js']};
