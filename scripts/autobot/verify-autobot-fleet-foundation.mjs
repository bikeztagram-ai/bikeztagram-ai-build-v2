...
for(const file of [repairFile,qaFile,reviewerFile,repairVerifierFile,qaVerifierFile,reviewerVerifierFile])assert(fs.existsSync(path.join(root,file)),`registered path must exist: ${file}`);
