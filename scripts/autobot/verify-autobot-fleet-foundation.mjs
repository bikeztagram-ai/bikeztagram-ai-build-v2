...
assert(repair.includes("'install','--no-audit','--no-fund','--no-package-lock'"),'Repair Bot must install dependencies inside its isolated worktree');
assert(qa.includes("'install','--no-audit','--no-fund','--no-package-lock'"),'QA Bot must install dependencies inside its isolated worktree');
assert(reviewer.includes("'install','--no-audit','--no-fund','--no-package-lock'"),'Reviewer must install dependencies inside its isolated worktree');
assert(selfImprovement.includes('aider-feature-brain-state.json'),'Self-Improvement must consume the same resumable Builder state as the protected Builder');
...
assert(doc.includes('builder/working/aider-feature-brain-state.json'),'foundation documentation must name the protected Builder resumable state path');
