...
assert(productionSpecialists.length===2,'exactly two verified specialist Builders are authorised by the current live gate');
assert(JSON.stringify(productionSpecialists.map(b=>b.id))===JSON.stringify(['director-builder','timeline-builder']),'production specialist activation set changed');
const scopes={'director-builder':['src/director.js'],'timeline-builder':['src/executableTimeline.js','src/editorialRhythm.js','src/renderer.js'],'music-builder':['src/musicDirector.js'],'scene-builder':['src/universalCreativeSceneEngine.js']};
