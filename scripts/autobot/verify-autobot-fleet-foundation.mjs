...
assert(Number.isInteger(registry.coordination?.maxConcurrentWorkers)&&registry.coordination.maxConcurrentWorkers>=1&&registry.coordination.maxConcurrentWorkers<=6,'fleet worker limit must remain bounded at six');
if(live){
 ...
 assert(JSON.stringify(registry.activationGate?.parallelWorkers||[])===JSON.stringify(['director-builder','timeline-builder','music-builder','scene-builder','rhythm-builder','render-builder']),'parallel activation must name exactly the six isolated production specialists');
...
assert(productionSpecialists.length===6,'exactly six verified specialist Builders are authorised by the current live gate');
