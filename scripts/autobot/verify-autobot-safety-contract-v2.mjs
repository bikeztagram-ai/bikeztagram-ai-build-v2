...
if (!feature.includes('dependency-blocked=')) failures.push('feature brain does not report dependency-blocked objectives');
if (!feature.includes('state.failed')) failures.push('feature brain does not persist failure state');
if (failures.length) { console.error(failures.map(f => `FAIL: ${f}`).join('\n')); process.exit(1); }
console.log(`AutoBot dependency contract PASS: ${objectives.length} objectives, ${objectives.reduce((n,o)=>n+(o.dependsOn||[]).length,0)} dependency edges, no cycles.`);
