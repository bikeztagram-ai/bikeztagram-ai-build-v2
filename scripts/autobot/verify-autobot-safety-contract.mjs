...
if (/GEMINI_API_KEY|gemini-cli|gemini-3/i.test(workflow + scheduler + sustained + feature + deterministic + gate)) failures.push('forbidden AI provider reference in active AutoBot runtime');
if (/gh\s+pr\s+merge|gh\s+pr\s+approve/i.test(workflow + sustained + feature + deterministic + gate)) failures.push('automatic merge/approval path detected');
if (/vercel\s+(deploy|promote)|vercel\.com\/api/i.test(workflow + sustained + feature + deterministic + gate)) failures.push('automatic production deployment path detected');
