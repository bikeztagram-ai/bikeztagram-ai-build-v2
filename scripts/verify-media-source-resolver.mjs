...
const uploadedFromBlobSourceUrl = resolveMediaSource({ sourceType: 'uploaded', mediaIndex: 0 }, [
  { sourceUrl: 'https://example.blob.vercel-storage.com/video-a.mp4', type: 'video/mp4' },
]);
assert.equal(uploadedFromBlobSourceUrl.ready, true);
assert.equal(uploadedFromBlobSourceUrl.url, 'https://example.blob.vercel-storage.com/video-a.mp4');
assert.equal(uploadedFromBlobSourceUrl.type, 'video/mp4');
