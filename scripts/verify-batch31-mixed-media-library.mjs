...
assert.match(app,/multiple onChange/);assert.match(app,/slice\(0,12\)/);assert.match(app,/\/api\/analyse-library/);assert.match(app,/mediaId:generated\?undefined:\(source\?\.id\|\|`source-\$\{sourceIndex\}`\)/);assert.match(app,/renderInspectImprove/);assert.match(app,/mediaItems:sources/);assert.match(endpoint,/sourceIndex/);assert.match(endpoint,/createPartFromUri/);assert.match(endpoint,/Maximum source-library size is 12/);assert.match(endpoint,/subject may be anything/);assert.match(endpoint,/Do not assume the subject from filenames/);assert.match(upload,/image\/jpeg/);assert.match(upload,/video\/mp4/);assert.match(upload,/maximumSizeInBytes/);
