...
async function main(){
  ...
  const response=await fetch(`${host}/api/generate`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model,prompt,stream:false,format:'json',options:{temperature:0.25}})});if(!response.ok)throw new Error(`planner model HTTP ${response.status}`);const body=await response.json();const raw=clean(body.response);const byId=new Map((raw.packages||[]).map(p=>[String(p.botId),p]));const seenTitles=new Set(),seenFiles=new Set();const packages=bots.map(bot=>{const item=byId.get(bot.id);if(!item)throw new Error(`planner omitted ${bot.id}`);return validate(item,bot,seenTitles,seenFiles,inv,library);});
