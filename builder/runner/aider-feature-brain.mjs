...
function recentLearning(obj){return{failures:(learning.failures||[]).filter(x=>x.objective===obj.id).slice(-6),successes:(learning.successes||[]).filter(x=>x.objective===obj.id).slice(-4)};}
function promptFor(obj,pass){const files=scopedFiles(obj),newFiles=allowNewFiles(obj),self=obj.kind==='self-improvement',feedback=recentLearning(obj),learningBrief=self?buildSelfImprovementBrief(learning):null,focus=process.env.AUTOBOT_FEATURE_FOCUS||'';return ['You are the Bikeztagram AI autonomous feature engineer.',`Objective: ${obj.title||obj.id}`,`Objective kind: ${obj.kind||'product'}.`,`Pass ${pass} of ${maxPasses}.`,focus?`RUN FOCUS: ${focus}`:'',`Acceptance criteria: ${JSON.stringify(obj.acceptance||[])}`,`Objective-scoped files: ${files.join(', ')||'(none)'}`,`Explicitly allowlisted new files: ${newFiles.join(', ')||'(none)'}`,`Protected paths: ${protectedPaths(obj).join(', ')}`,`Objective constraints: ${JSON.stringify(obj.constraints||[])}`,self?`Self-improvement evidence and lessons: ${JSON.stringify(learningBrief)}`:`Relevant prior run feedback: ${JSON.stringify(feedback)}`,'Inspect the supplied files and relevant callers/contracts as needed. Make one coherent, real engineering improvement for this objective.',self?'This is a controlled self-improvement task. Improve the autonomous engineering mechanism using the recorded evidence. Do not weaken any safety, scope, rollback, audit, production, verification, protected-path or no-auto-commit control.':'This is a product task. Do not modify the autonomous builder, workflows, safety controls or infrastructure.','Only objective files and explicitly allowlisted new files may be modified. Protected paths are read-only.','Do not widen objective scope, alter objective definitions, alter protected-path policy, or grant additional permissions.','Preserve public contracts, Gemini-free/provider-neutral product runtime, rollback and verification gates.','Run the narrowest relevant verification. Actually edit the allowed files; do not merely describe a solution.','If previous feedback identifies a failure, explicitly address that failure or add a guard/test that prevents recurrence.','Do not merge or create a pull request.'].filter(Boolean).join('\n');}
function assertScope(before,obj){const allowed=allowedPaths(obj),after=trackedPaths();const changed=after.filter(p=>!before.has(p));const violations=[...new Set([...changed.filter(p=>!allowed.has(p)),...after.filter(p=>isProtected(p,obj))])];if(violations.length){for(const file of violations)restorePath(file);throw new Error(`scope/protection violation: ${violations.join(', ')}`);}}
...
let success=false;
for(let pass=1;pass<=maxPasses;pass++){if(remainingMs()<35_000)break;state.runs=(state.runs||0)+1;learning.runs=(learning.runs||0)+1;const before=new Set(trackedPaths());const timeout=Math.min(perCallMaxMs,remainingMs()-5_000);const apiTimeout=Math.max(30,Math.floor(timeout/1000));const args=[`--model=${model}`,`--timeout=${apiTimeout}`,'--yes-always','--no-auto-commits','--no-dirty-commits','--no-gitignore','--no-show-model-warnings','--map-tokens=512','--subtree-only','--message',promptFor(obj,pass),...aiderFiles];const result=spawnSync('aider',args,{cwd:aiderCwd,encoding:'utf8',stdio:'inherit',timeout});if(result.error){const error=new Error(result.error.code||result.error.message);recordFailure(obj,pass,error);state.failed=[...(state.failed||[]),{id:obj.id,pass,code:result.error.code||'process-error'}];persist();continue;}if(result.status!==0){const error=new Error(`Aider exited with status ${result.status}`);recordFailure(obj,pass,error);state.failed=[...(state.failed||[]),{id:obj.id,pass,code:result.status}];persist();continue;}try{assertScope(before,obj);verifyDiff();if(obj.kind==='self-improvement')execFileSync('npm',['run','verify:autobot-self-improvement-boundary'],{cwd:root,stdio:'inherit'});verifyBuild();recordSuccess(obj,pass,before);success=true;break;}catch(error){console.error(`[aider] pass ${pass} verification failed: ${error.message}`);recordFailure(obj,pass,error);state.failed=[...(state.failed||[]),{id:obj.id,pass,code:'verification',error:String(error.message).slice(0,500)}];persist();if(remainingMs()<35_000)break;}}
...
persist();console.log(JSON.stringify({ok:success,protocol,objective:obj.id,kind:obj.kind||'product',passes:maxPasses,model,elapsedMs:(requestedMinutes*60_000)-remainingMs(),remainingMs:remainingMs(),learningFailures:(learning.failures||[]).length,learningSuccesses:(learning.successes||[]).length}));process.exit(success?0:1);
