...
function restorePath(file){try{execFileSync('git',['restore','--',file],{cwd:root,stdio:'inherit'});}catch{}try{execFileSync('git','clean'.split(' ').concat(['-fd','--',file]),{cwd:root,stdio:'inherit'});}catch{}}
...
function focusInstruction(file,obj,pass){if(obj.id==='autobot-self-performance'){if(file.includes('aider-feature-brain'))return 'Focus on the Aider execution loop: make model attempts more targeted and more likely to produce a real verified edit within the bounded budget. Do not merely add logging or rewrite unrelated logic.';if(file.includes('ollama-performance-proxy'))return 'Focus on hosted Ollama request reliability and bounded inference behaviour. Make one concrete change that reduces wasted model time without weakening correctness or safety.';if(file.includes('self-improvement-planner'))return 'Focus on turning observed timeout/no-progress evidence into a concrete, deterministic next-step instruction for the next self-improvement attempt.';}return `Focus only on ${file}; make one small, complete improvement relevant to objective ${obj.id}.`;}
...
const focusFileByPass=pass=>chooseFocusFile(obj,pass);
let success=false;
for(let pass=1;pass<=maxPasses;pass++){
  ...
  const focusFile=focusFileByPass(pass);
  const focusRelative=focusFile.startsWith('builder/runner/')?focusFile.slice('builder/runner/'.length):focusFile;
  const aiderCwd=focusFile.startsWith('builder/runner/')?path.join(root,'builder/runner'):root;
