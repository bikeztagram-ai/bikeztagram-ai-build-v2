...
const protocol='aider-repo-map-v6-bounded-learning';
...
const configuredCallMax=Math.max(60_000,Number.parseInt(process.env.AUTOBOT_AIDER_CALL_TIMEOUT_MS||String(5*60*1000),10));
const reserveMs=Math.max(30_000,Number.parseInt(process.env.AUTOBOT_VERIFICATION_RESERVE_MS||'75_000',10));
...
function rollbackAttempt(before,obj){const beforeSet=new Set(before);for(const file of trackedPaths().filter(p=>!beforeSet.has(p))){if(allowedPaths(obj).has(file))restorePath(file);}}
...
function promptFor(obj,pass,attemptMs){const files=scopedFiles(obj),newFiles=allowNewFiles(obj),self=obj.kind==='self-improvement',feedback=recentLearning(obj),learningBrief=self?buildSelfImprovementBrief(learning):null,focus=process.env.AUTOBOT_FEATURE_FOCUS||'';return ['You are the Bikeztagram AI autonomous feature engineer.',`Objective: ${obj.title||obj.id}`,`Objective kind: ${obj.kind||'product'}.`,`Pass ${pass} of ${maxPasses}.`,`This attempt has a hard execution budget of ${Math.round(attemptMs/1000)} seconds; make the smallest complete improvement that can be implemented and verified within that budget.`,focus?`RUN FOCUS: ${focus}`:'',`Acceptance criteria: ${JSON.stringify(obj.acceptance||[])}`,`Objective-scoped files: ${files.join(', ')||'(none)'}`,`Explicitly allowlisted new files: ${newFiles.join(', ')||'(none)'}`,`Protected paths: ${protectedPaths(obj).join(', ')}`,`Objective constraints: ${JSON.stringify(obj.constraints||[])}`,self?`Self-improvement evidence and lessons: ${JSON.stringify(learningBrief)}`:`Relevant prior run feedback: ${JSON.stringify(feedback)}`,'Inspect the supplied files and relevant callers/contracts as needed. Make one coherent, real engineering improvement for this objective.',self?'This is a controlled self-improvement task. Improve the autonomous engineering mechanism using the recorded evidence. In particular, address repeated timeout/no-progress behaviour when supported by the evidence. Do not weaken any safety, scope, rollback, audit, production, verification, protected-path or no-auto-commit control.':'This is a product task. Do not modify the autonomous builder, workflows, safety controls or infrastructure.','Only objective files and explicitly allowlisted new files may be modified. Protected paths are read-only.','Do not widen objective scope, alter objective definitions, alter protected-path policy, or grant additional permissions.','Preserve public contracts, Gemini-free/provider-neutral product runtime, rollback and verification gates.','Do not spend the attempt merely explaining a solution. Actually edit an allowed file and finish a bounded change.','If previous feedback identifies a failure, explicitly address that failure or add a guard/test that prevents recurrence.','Do not merge or create a pull request.'].filter(Boolean).join('\n');}
...
function classifyFailure(error){const message=String(error?.message||error||'unknown failure').toLowerCase();if(message.includes('scope')||message.includes('protected'))return'scope-violation';if(message.includes('build'))return'build-failure';if(message.includes('diff'))return'diff-failure';if(message.includes('boundary'))return'boundary-failure';if(message.includes('timeout')||message.includes('etimedout'))return'timeout';if(message.includes('no-progress'))return'no-progress';return'verification-failure';}
...
for(let pass=1;pass<=maxPasses;pass++){
  ...
  if(result.error){const error=new Error(result.error.code||result.error.message);rollbackAttempt(before,obj);recordFailure(obj,pass,error);state.failed=[...(state.failed||[]),{id:obj.id,pass,code:result.error.code||'process-error'}];persist();continue;}
  if(result.status!==0){const error=new Error(`Aider exited with status ${result.status}`);rollbackAttempt(before,obj);recordFailure(obj,pass,error);state.failed=[...(state.failed||[]),{id:obj.id,pass,code:result.status}];persist();continue;}
  try{
