...
for(let pass=first;pass<=maxPasses;pass++){
  ...
  const args=[`--model=${model}`,`--timeout=${Math.max(30,Math.floor(timeout/1000))}`,'--yes-always','--no-auto-commits','--no-dirty-commits','--no-gitignore','--no-show-model-warnings',...(specialist?['--map-tokens=2048','--subtree-only',`--edit-format=udiff`]:[`--map-tokens=768`,'--subtree-only','--edit-format=whole']),'--message',promptFor(o,pass),...aiderFiles];
