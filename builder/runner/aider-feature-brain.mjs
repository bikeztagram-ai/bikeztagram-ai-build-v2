...
function assertScope(before,obj){const allowed=allowedPaths(obj),after=trackedPaths();const changed=after.filter(p=>!before.has(p));const violations=[...new Set([...changed.filter(p=>!allowed.has(p)),...after.filter(p=>isProtected(p,obj))])];if(violations.length){rollbackAttempt(before);throw new Error(`scope/protection violation: ${violations.join(', ')}`);}}
