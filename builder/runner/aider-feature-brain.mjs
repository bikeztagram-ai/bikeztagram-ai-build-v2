...
// Never let one model call consume an entire feature slice. This hard ceiling
// preserves time for a second/third attempt plus independent verification.
const configuredCallMax=Math.min(300_000,Math.max(60_000,Number.parseInt(process.env.AUTOBOT_AIDER_CALL_TIMEOUT_MS||String(5*60*1000),10)));
...
function chooseFocusFile(obj,pass){const files=scopedFiles(obj);if(files.length<=1)return files[0];const feedback=recentLearning(obj);const latest=feedback.failures.at(-1);const preferred=latest?.category==='timeout'||latest?.category==='no-progress';if(preferred&&files.some(f=>f.includes('aider-feature-brain')))return files.find(f=>f.includes('aider-feature-brain'));return files[(pass-1)%files.length];}
function focusInstruction(file,obj,pass){if(obj.id==='autobot-self-performance'){if(file.includes('aider-feature-brain'))return 'Focus on the Aider execution loop. Make ONE micro-change (ideally about 5-20 lines) that makes a model attempt more likely to produce a real verified edit within the bounded budget. Do not refactor, add logging, or rewrite unrelated logic. Stop after the concrete edit.';if(file.includes('ollama-performance-proxy'))return 'Focus on hosted Ollama request reliability and bounded inference behaviour. Make ONE small concrete change that reduces wasted model time without weakening correctness or safety.';if(file.includes('self-improvement-planner'))return 'Focus on turning observed timeout/no-progress evidence into ONE concrete, deterministic next-step instruction for the next self-improvement attempt.';}return `Focus only on ${file}; make one small, complete improvement relevant to objective ${obj.id}.`;}
function promptFor(obj,pass,attemptMs,file){const files=scopedFiles(obj),newFiles=allowNewFiles(obj),self=obj.kind==='self-improvement',feedback=recentLearning(obj),learningBrief=self?buildSelfImprovementBrief(learning):null,focus=process.env.AUTOBOT_FEATURE_FOCUS||'';const compactLearning=self?{recommendedNextAction:learningBrief?.recommendedNextAction,highestPriorityLearning:learningBrief?.highestPriorityLearning,latestFailure:learningBrief?.latestFailure,efficiencySignals:learningBrief?.efficiencySignals,recentFailures:learningBrief?.recentFailures?.slice(-3)}:feedback;return ['You are the Bikeztagram AI autonomous feature engineer.',`Objective: ${obj.title||obj.id}`,`Objective kind: ${obj.kind||'product'}.`,`Pass ${pass} of ${maxPasses}.`,`Hard execution budget: ${Math.round(attemptMs/1000)} seconds. Make the smallest complete improvement that can be implemented and verified within it.`,`Primary coding model policy: use the strongest configured local coding model for substantive engineering; do not trade away correctness merely to reduce startup or inference time.`,focus?`RUN FOCUS: ${focus}`:'',`Acceptance criteria: ${JSON.stringify(obj.acceptance||[])}`,`Objective-scoped files: ${files.join(', ')||'(none)'}`,`Current pass focus file: ${file||'(none)'}`,`Explicitly allowlisted new files: ${newFiles.join(', ')||'(none)'}`,`Protected paths: ${protectedPaths(obj).join(', ')}`,`Constraints: ${JSON.stringify(obj.constraints||[])}`,self?`Self-improvement evidence: ${JSON.stringify(compactLearning)}`:`Prior run feedback: ${JSON.stringify(compactLearning)}`,focusInstruction(file,obj,pass),'Inspect the focus file and only the directly relevant callers/contracts needed to make the change.',self?'This is a controlled self-improvement task. Use the evidence to improve the autonomous engineering mechanism. Address repeated timeout/no-progress behaviour when supported by evidence. Do not weaken safety, scope, rollback, audit, production, verification, protected-path or no-auto-commit controls.':'This is a product task. Do not modify autonomous-builder infrastructure or safety controls.','Only objective files and explicitly allowlisted new files may be modified. Protected paths are read-only.','Do not widen objective scope, alter objective definitions, alter protected-path policy, or grant permissions.','Preserve public contracts, Gemini-free/provider-neutral product runtime, rollback and verification gates.','Do not spend the attempt merely explaining a solution. Actually edit the focus file and finish a bounded change.','Do not merge or create a pull request.'].filter(Boolean).join('\n');}
...
function recordFailure(obj,pass,error,file){learning.failures=[...(learning.failures||[]),{objective:obj.id,kind:obj.kind||'product',pass,focusFile:file,category:classifyFailure(error),message:String(error?.message||error).slice(0,500),at:new Date().toISOString()}].slice(-100);}
function recordSuccess(obj,pass,before,file){learning.successes=[...(learning.successes||[]),{objective:obj.id,kind:obj.kind||'product',pass,focusFile:file,changedPaths:trackedPaths().filter(p=>!before.has(p)),at:new Date().toISOString()}].slice(-100);}
...
try{validateObjective(obj);}catch(error){recordFailure(obj,0,error,'(contract)');persist();console.error(`[aider] invalid objective contract: ${error.message}`);process.exit(1);}
...
for(let pass=1;pass<=maxPasses;pass++){
  ...
  if(baseline.length){const error=new Error(`dirty-worktree-before-attempt: ${baseline.join(', ')}`);recordFailure(obj,pass,error,'(worktree)');persist();break;}
  ...
  const focusFile=chooseFocusFile(obj,pass);
  const focusRelative=scopePrefix&&focusFile.startsWith(scopePrefix)?focusFile.slice(scopePrefix.length):focusFile;
  const aiderCwd=scopePrefix&&focusFile.startsWith(scopePrefix)?path.join(root,scopePrefix):root;
  const args=[`--model=${model}`,`--timeout=${apiTimeout}`,'--yes-always','--no-auto-commits','--no-dirty-commits','--no-gitignore','--no-show-model-warnings','--map-tokens=256','--subtree-only','--edit-format=diff','--message',promptFor(obj,pass,attemptMs,focusFile),focusRelative];
  console.log(`[aider] pass ${pass}/${maxPasses}; focus=${focusFile}; budget=${Math.round(attemptMs/1000)}s`);
  const result=spawnSync('aider',args,{cwd:aiderCwd,encoding:'utf8',stdio:'inherit',timeout:attemptMs+5_000});
  if(result.error){const error=new Error(result.error.code||result.error.message);rollbackAttempt(before);recordFailure(obj,pass,error,focusFile);state.failed=[...(state.failed||[]),{id:obj.id,pass,focusFile,code:result.error.code||'process-error'}];persist();continue;}
  if(result.status!==0){const error=new Error(`Aider exited with status ${result.status}`);rollbackAttempt(before);recordFailure(obj,pass,error,focusFile);state.failed=[...(state.failed||[]),{id:obj.id,pass,focusFile,code:result.status}];persist();continue;}
  try{
    ...
    verifyDiff();
    if(isSelfImprovement)execFileSync('npm',['run','verify:autobot-self-improvement-boundary'],{cwd:root,stdio:'inherit'});
    verifyBuild();
    recordSuccess(obj,pass,before,focusFile);success=true;break;
  }catch(error){
    console.error(`[aider] pass ${pass} verification failed: ${error.message}`);
    rollbackAttempt(before);
    recordFailure(obj,pass,error,focusFile);
    state.failed=[...(state.failed||[]),{id:obj.id,pass,focusFile,code:'verification',error:String(error.message).slice(0,500)}];
    persist();
