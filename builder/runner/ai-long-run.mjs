 ...
 * until the shared time budget expires or the bounded pass budget is exhausted.
 */
...
const maxPasses = Math.min(Number.parseInt(process.env.AUTOBOT_CODEX_PASSES || '8', 10), 8);
...
if (!process.env.OPENAI_API_KEY) {
  console.log('[autobot] OPENAI_API_KEY is unavailable; AI engineering layer will not run.');
  process.exit(0);
}
...
const build = () => {
  const result = spawnSync('npm', ['run', 'build'], { cwd: root, stdio: 'inherit', env: process.env, timeout: Math.max(60_000, Math.min(minutesLeft() * 60_000, 15 * 60_000)) });
...
while (pass < maxPasses && minutesLeft() > 1) {
  ...
  const prompt = [
    ...
    'Do not spend the pass on cosmetic refactors, documentation-only work, duplicate tests, or builder infrastructure unless it is necessary to unlock product quality.',
    ...
    `This is Codex pass ${pass}/${maxPasses}; shared time remaining is about ${minutesLeft().toFixed(0)} minutes.`,
  ...
  try { execFileSync('git', ['diff', '--check'], { cwd: root, stdio: 'inherit' }); previousFailure = ''; }
  catch (error) { previousFailure = 'git diff --check failed; fix whitespace/conflict issues before continuing.'; continue; }

  const after = gitStatus();
  if (after !== before) {
    if (!build()) previousFailure = 'npm run build failed after this pass; fix the build before the next pass.';
    else console.log(`[autobot] Codex pass ${pass} produced verified product changes.`);
  } else {
    console.log(`[autobot] Codex pass ${pass} produced no new diff; continuing rather than declaring success.`);
  }
}

console.log(`[autobot] AI engineering layer finished: ${pass} Codex passes, ${((Date.now() - started) / 60000).toFixed(2)} minutes elapsed.`);
