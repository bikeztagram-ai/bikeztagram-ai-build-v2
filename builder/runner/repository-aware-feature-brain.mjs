...
function modelCall(messages, readToolEnabled = true) {
...
function executeObjective(objective, repair) {
  ...
  let editCount = 0; let submitted = false; let summary = ''; let inspected = false; let readToolEnabled = true; let emptyTurns = 0; let failedEditAttempts = 0; const failedEditFiles = new Set();
  ...
  for (let turn = 1; turn <= maxTurns && left() > 0.75; turn += 1) {
    ...
    let response;
    try { response = modelCall(messages, readToolEnabled); }
    catch (error) {
    ...
    for (const call of calls) {
      ...
      if (call.name === 'submit') { submitted = true; summary = String(call.args.summary || '').slice(0, 700); result = 'SUBMIT RECEIVED'; }
      else if (call.name === 'read_file') { inspected = true; readToolEnabled = false; result = readFileWindow(call.args.file, call.args.start, call.args.end, objective); }
        ...
        else { result = editFile(call.args.file, call.args.search, call.args.replace, objective); if (result.startsWith('EDIT APPLIED')) { editCount += 1; readToolEnabled = false; } else { failedEditAttempts += 1; failedEditFiles.add(String(call.args.file || '')); readToolEnabled = true; } }
