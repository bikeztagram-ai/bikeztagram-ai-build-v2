...
function executeObjective(objective, repair) {
  ...
  for (let turn = 1; turn <= maxTurns && left() > 0.75; turn += 1) {
    ...
    for (const call of calls) {
      ...
      if (call.name === 'submit') { submitted = true; summary = String(call.args.summary || '').slice(0, 700); result = 'SUBMIT RECEIVED'; }
      else if (call.name === 'read_file') { inspected = true; toolPhase = 'edit'; result = readFileWindow(call.args.file, call.args.start, call.args.end, objective); }
        ...
        else { result = editFile(call.args.file, call.args.search, call.args.replace, objective); if (result.startsWith('EDIT APPLIED')) { editCount += 1; toolPhase = 'verify'; } else { failedEditAttempts += 1; failedEditFiles.add(String(call.args.file || '')); toolPhase = 'inspect'; } }
