...
function modelCall(messages, toolPhase = 'inspect') {
...
function executeObjective(objective, repair) {
  ...
  let editCount = 0; let submitted = false; let summary = ''; let inspected = false; let toolPhase = 'inspect'; let emptyTurns = 0; let failedEditAttempts = 0; const failedEditFiles = new Set();
  ...
  const basePrompt = [
    ...
    '',
    'EXECUTION CONTRACT:',
    'First inspect one supplied file with read_file. Then STOP INSPECTING and call edit_file with one precise, meaningful improvement. For a new top-level helper prefer mode=insert_before with a short unique declaration anchor; use insert_after only for a complete top-level statement; use replace only for a small complete syntactic unit. Never invent an enclosing function body.',
  ...
  for (let turn = 1; turn <= maxTurns && left() > 0.75; turn += 1) {
    ...
    let response;
    try { response = modelCall(messages, toolPhase); }
    catch (error) {
    ...
    for (const call of calls) {
      ...
      if (call.name === 'submit') { submitted = true; summary = String(call.args.summary || '').slice(0, 700); result = 'SUBMIT RECEIVED'; }
      else if (call.name === 'read_file') { inspected = true; failedEditFiles.delete(String(call.args.file || '')); toolPhase = 'edit'; result = readFileWindow(call.args.file, call.args.start, call.args.end, objective); }
      else if (call.name === 'git_status') result = 'ERROR: git_status is not available during a Qwen feature turn.';
      else if (call.name === 'git_diff') result = 'ERROR: git_diff is not available during a Qwen feature turn.';
      else if (call.name === 'run_check') { result = runCheck(call.args.check); toolPhase = result === 'PASS' ? 'submit' : 'inspect'; }
        ...
        else { result = call.args.mode === 'insert_before' ? insertBefore(call.args.file, call.args.search, call.args.replace, objective) : call.args.mode === 'insert_after' ? insertAfter(call.args.file, call.args.search, call.args.replace, objective) : editFile(call.args.file, call.args.search, call.args.replace, objective); if (result.startsWith('EDIT APPLIED')) { editCount += 1; toolPhase = 'verify'; } else { failedEditAttempts += 1; failedEditFiles.add(String(call.args.file || '')); toolPhase = 'inspect'; } }
