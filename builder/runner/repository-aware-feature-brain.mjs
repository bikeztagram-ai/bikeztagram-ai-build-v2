...
const PROTOCOL = 'repository-aware-agent-v6';
...
function saveState() { fs.mkdirSync(path.dirname(statePath), { recursive: true }); fs.writeFileSync(statePath, JSON.stringify({ version: 12, completed: [], progress, failed: state.failed || {}, updatedAt: new Date().toISOString() }, null, 2) + '\n'); }
...
function repositoryMap(objective) {
  ...
  const files = (repo.files || []).filter((file) => !isSensitive(file.path) && (objectiveFiles.has(file.path) || file.path.startsWith('src/') || file.path.startsWith('api/'))).slice(0,60);
  const edges = (repo.dependencyEdges || []).filter((edge) => objectiveFiles.has(edge.from)).slice(0,60);
  return JSON.stringify({summary:repo.summary,files,dependencyEdges:edges},null,2).slice(0,7000);
}
function listFiles(prefix='') { const files=(repo.files||[]).filter((file)=>!isSensitive(file.path)&&(!prefix||file.path.startsWith(prefix))).slice(0,300); return files.map((file)=>`${file.path} — ${file.lines||0} lines — ${file.purpose||''}`).join('\n')||'No indexed files matched.'; }
function searchRepo(query) { const value=String(query||'').trim(); if(!value)return'ERROR: search query is required.'; const result=capture('git',['grep','-n','-I','-F','--',value,':!builder/working',':!.env',':!.env.example']); return result.trim().slice(0,6000)||`No matches for: ${value}`; }
...
function modelCall(messages) {
  ...
  const body=JSON.stringify({model,stream:false,keep_alive:'15m',think:false,tools,options:{temperature:0,num_ctx:4096,num_predict:900},messages});
...
function parseToolCalls(response){
  const native=(response?.message?.tool_calls||[]).map((call)=>({name:call.function?.name,args:typeof call.function?.arguments==='string'?JSON.parse(call.function.arguments):(call.function?.arguments||{})}));
  if(native.length)return native;
  const content=String(response?.message?.content||'');
  const fallback=[];
  for(const match of content.matchAll(/<tool_call>\s*(\{[\s\S]*?\})\s*<\/tool_call>/g)){try{const parsed=JSON.parse(match[1]);if(parsed?.name)fallback.push({name:parsed.name,args:parsed.arguments||{}});}catch{}}
  return fallback;
}
function executeObjective(objective,repair){
  ...
  let editCount=0,submitted=false,summary='',emptyTurns=0;
  const prompt=['You are the senior autonomous engineer for Bikeztagram AI. Work on the real repository using tools; do not merely describe a solution.',`OBJECTIVE: ${objective.title}`,`PRIORITY: ${objective.priority}`,`VERIFIED PROGRESS: ${progress[objective.id]||0}`,`DEPENDENCIES: ${(objective.dependsOn||[]).join(', ')||'none'}`,'','REPOSITORY KNOWLEDGE:',repositoryMap(objective),'','ACCEPTANCE:',...(objective.acceptance||[]).map((item)=>`- ${item}`),'CONSTRAINTS:',...(objective.constraints||[]).map((item)=>`- ${item}`),'OBJECTIVE WRITE SCOPE:',...(objective.files||[]).map((file)=>`- ${file}`),'','IMPORTANT: Start by calling a repository tool. Do not answer with a plan or prose. Inspect first, then edit, verify, repair if needed, and submit. Read broadly, write narrowly. Use at most the edit budget. Never reset or clean the working tree. Never modify builder infrastructure, workflows, secrets, dependencies, Vercel config, or Gemini integration.',repair?'This is a repair attempt: inspect the current state and fix the previous failure rather than repeating it.':''].join('\n');
  const messages=[{role:'system',content:'You are a multi-turn tool-using senior software engineer. Every useful turn should call a tool. Explore, edit, test, diagnose, repair, and submit verified code.'},{role:'user',content:prompt}];
  for(let turn=1;turn<=maxTurns&&left()>0.75;turn+=1){
    ...
    let response;
    try{response=modelCall(messages);}catch(error){console.error(`[autobot] model call failed: ${error.message}`);if(turn<maxTurns){messages.push({role:'user',content:'The previous request failed or timed out. Do not explain. Make the next response a direct repository tool call, starting with repository_map or read_file.'});continue;}break;}
    const assistant=response.message||{role:'assistant',content:''};
    messages.push(assistant);
    const calls=parseToolCalls(response);
    if(!calls.length){
      emptyTurns+=1;
      if(emptyTurns>=2){console.error('[autobot] model produced prose without a usable tool call twice; stopping this attempt.');break;}
      messages.push({role:'user',content:'No tool call was received. Do not write prose. Call repository_map, read_file, search_repo, edit_file, run_check, or submit now.'});
      continue;
    }
    emptyTurns=0;
    for(const call of calls){
      if(call.name==='submit'){submitted=true;summary=String(call.args.summary||'').slice(0,1000);messages.push({role:'tool',content:'Submission received. Runner verification is authoritative.'});continue;}
      let result;
      ...
      else if(call.name==='edit_file'){if(editCount>=maxEdits)result=`ERROR: edit budget exhausted (${maxEdits}). Verify or submit.`;else{result=editFile(call.args.file,call.args.search,call.args.replace,objective);if(result.startsWith('EDIT APPLIED'))editCount+=1;}}
      else result=`ERROR: unknown tool ${call.name}`;
      console.log(`[autobot] ${call.name}: ${result.slice(0,800).replace(/\n/g,' ')}`);
      messages.push({role:'tool',content:result.slice(0,7000)});
    }
    if(submitted)break;
  }
  if(!submitted){for(const [file,snapshot] of snapshots){const current=read(file);if(current!==snapshot)fs.writeFileSync(abs(file),snapshot);}return{ok:false,reason:'agent did not submit',edits:editCount};}
  const diff=gitDiff();
  if(!diff.trim()||editCount<1){for(const [file,snapshot] of snapshots){if(read(file)!==snapshot)fs.writeFileSync(abs(file),snapshot);}return{ok:false,reason:'submission had no verified product diff',edits:editCount};}
  const build=runCheck('build'),diffCheck=runCheck('diff-check');
