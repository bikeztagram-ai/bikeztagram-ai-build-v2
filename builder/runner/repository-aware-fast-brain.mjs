...
function modelPatch(o) {
  const schema = {
    ...
    properties: {
      edits: {
        type: 'array', maxItems: 1, minItems: 1,
        items: {
          type: 'object', additionalProperties: false,
          properties: {
            file: { type: 'string' },
            search: { type: 'string', minLength: 12 },
  ...
  const body = JSON.stringify({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a careful senior engineer editing an existing JavaScript product. Return one tiny verified-safe patch. Do not plan. Do not rewrite files. Never touch infrastructure.'
      },
      {
        role: 'user',
        content: [
...
try {
  result = apply(objective, modelPatch(objective));
} catch (error) {
