...
function live(state, message, verification = []) {
  writeLiveState({ runId: process.env.GITHUB_RUN_ID || 'local', status: state.status, objectiveId: state.objectiveId, currentTask: state.currentTask, completedUnits: state.completed.length, totalUnits: state.totalUnits || 0, startedAt, message, files: state.files || [], verification, blockedReason: state.error || null });
}
...
if (!objective) {
  ...
  writeCheckpoint(state); live(state, 'No eligible roadmap objective. Idle.');
  process.exit(0);
}
...
if (!tasks.length) {
  state.status = 'blocked'; state.error = 'No ready deterministic tasks'; writeCheckpoint(state); live(state, state.error); throw new Error(`[autobot] ${state.error} for ${objective.id}`);
}
...
for (const task of tasks) {
  ...
  if (!allowedTask(task)) {
    state.status = 'blocked'; state.currentTask = task.id; state.error = 'Protected-path task rejected'; writeCheckpoint(state); live(state, state.error); throw new Error(`[autobot] ${state.error}: ${task.id}`);
  }
  if (!Array.isArray(task.implementation) || task.implementation.length === 0) {
    state.status = 'blocked'; state.currentTask = task.id; state.error = 'Task has no declared implementation commands'; writeCheckpoint(state); live(state, state.error); throw new Error(`[autobot] Refusing no-op task: ${task.id}`);
  }
  const unmet = (task.dependsOn || []).filter(dep => !completed.includes(dep));
  if (unmet.length) continue;
  state.currentTask = task.id; state.status = 'running';
  ...
  try {
    ...
    if (before === after) throw new Error('No repository change detected; refusing to mark task complete.');
    unitEvidence.after = after; unitEvidence.verification = verification; unitEvidence.completedAt = new Date().toISOString();
    ...
    unitEvidence.failedAt = new Date().toISOString(); unitEvidence.error = error.message; evidence.units.push(unitEvidence); fs.writeFileSync(evidencePath, JSON.stringify(evidence, null, 2) + '\n');
...
state.currentTask = null; state.status = completed.length === tasks.length ? 'objective-complete' : 'checkpointed';
writeCheckpoint(state); live(state, state.status === 'objective-complete' ? 'All deterministic units verified.' : 'Run limit reached; safe checkpoint created.');
