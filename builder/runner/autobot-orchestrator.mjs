...
function eligibleObjective(objectives,state){
...
function specialistFor(obj,registry){
  ...
  return registry.specialists.find(bot=>['proven','verified'].includes(bot.status)&&bot.id!=='repair'&&bot.id!=='qa'&&bot.id!=='reviewer'&&bot.id!=='self-improvement'&&Array.isArray(bot.files)&&bot.files.some(f=>wanted.has(f)))||registry.specialists.find(bot=>bot.id==='builder'&&['proven'].includes(bot.status))||null;
}
...
function main(){
  ...
  const library=json(objectivePath,{objectives:[]}); const state=json(statePath,{version:1,completedObjectives:[],assignments:[],failures:[]});
  let objective=eligibleObjective(library.objectives||[],state); let source='objective-library';
  ...
  const assignment={run,assignedAt:new Date().toISOString(),source,objective:{id:objective.id||`discovered-${Date.now()}`,title:objective.title,files:objective.files||[],acceptance:objective.acceptance||[],constraints:objective.constraints||[]},specialist:{id:specialist.id,status:specialist.status,trust:specialist.trust,entrypoint:specialist.entrypoint}};
  state.lastAssignment=assignment; state.assignments=[...(state.assignments||[]),assignment].slice(-50); write(statePath,state);
  write(path.join(root,'builder/working/autobot-orchestrator-assignment.json'),assignment);
  console.log(JSON.stringify({ok:true,status:'assigned',source,objective:assignment.objective.title,specialist:specialist.id,trust:specialist.trust}));
}
main();
