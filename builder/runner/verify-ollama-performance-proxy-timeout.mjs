...
const child = spawn(process.execPath, [target], { env: { ...process.env, OLLAMA_PROXY_PORT: String(proxyPort), OLLAMA_UPSTREAM: `http://127.0.0.1:${upstreamPort}` }, stdio: ['ignore', 'pipe', 'pipe'] });
let output = '';
...
try {
  ...
  for (let i = 0; i < 50; i += 1) {
    ...
    try { const r = await fetch(`http://127.0.0.1:${proxyPort}/health`, { signal: AbortSignal.timeout(300) }); if (r.ok && (await r.text()).trim() === 'ok') { healthy = true; break; } } catch {}
    await new Promise(resolve => setTimeout(resolve, 100));
  }
  if (!healthy) throw new Error('proxy failed initial health check; output: ' + output.slice(-3000));
  const started = Date.now();
  const responsePromise = fetch(`http://127.0.0.1:${proxyPort}/api/chat`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: 'test', messages: [{ role: 'user', content: 'test' }] }) });
  const response = await Promise.race([responsePromise, new Promise((_, reject) => setTimeout(() => reject(new Error('proxy did not time out a hanging upstream within 13000ms')), 13000))]);
