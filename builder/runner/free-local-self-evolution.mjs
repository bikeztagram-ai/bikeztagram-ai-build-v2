 ...
 *
 * The model is deliberately given one self-improvement file and a bounded task.
 * It must return a unified diff. The runner independently applies, scopes and
 * verifies that diff, and rolls it back on any failure.
 */
...
const exec = (cmd, args) => execFileSync(cmd, args, {
  cwd: root,
  encoding: 'utf8',
  stdio: 'pipe',
});
...
function learningSnapshot() {
  ...
  return files.map(file => {
    ...
    try {
      return `${file}: ${JSON.stringify(JSON.parse(fs.readFileSync(full, 'utf8'))).slice(-9000)}`;
    } catch {
...
function assertScope(before) {
  ...
  const changed = after.filter(file => !beforeSet.has(file));
...
The previous hosted/local coding attempts repeatedly timed out with zero verified edits.
...
`;
  ...
  const evidence = {
    engine: 'free-local-qwen-llama.cpp',
    model,
    target,
    changedPaths: trackedPaths().filter(file => !before.includes(file)),
    diffBytes: Buffer.byteLength(finalDiff),
    verified: true,
    provider: 'local-only',
...
try {
  ...
  console.error(`[autobot-free] ${error.message}`);
  process.exit(1);
}
