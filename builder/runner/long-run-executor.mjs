...
function runOnce(minutes,units){
 ...
 console.log(`[autobot] sustained iteration ${iteration}: ${units} units / ${minutes.toFixed(2)} minutes remaining; completed objectives=${[...completedObjectives].join(',')||'none'}`);
...
console.log(`[autobot] Sustained run finished: ${totalUnits}/${requestedUnits} verified units; ${( (Date.now()-started)/60000 ).toFixed(2)} minutes elapsed; objectives completed=${[...completedObjectives].join(',')||'none'}.`);
