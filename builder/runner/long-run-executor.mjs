 ...
 * A deterministic task failure is recoverable: the feature brain gets a
 * chance to repair product code instead of the whole run dying immediately.
 */
...
function replenishBacklog() {
  if (replenishments >= maxReplenishments) return false;
...
function runFeatureBrain() {
  if (remainingMinutes() <= 1 || featureCycles >= maxFeatureCycles) return 0;
  featureCycles++;
  const slice = Math.min(featureSliceMinutes, Math.max(1, Math.floor(remainingMinutes())));
  const env = { ...process.env, BUILDER_MAX_MINUTES: String(slice), LOCAL_AI_MODEL: process.env.LOCAL_AI_MODEL || 'qwen2.5-coder:7b', AUTOBOT_FEATURE_PASSES: String(featurePassesPerSlice) };
  appendAudit('feature-brain-started', { cycle: featureCycles, minutes: slice, model: env.LOCAL_AI_MODEL, passes: featurePassesPerSlice, protocol: 'structured-search-replace-v4' });
  console.log(`[autobot] feature-engineering cycle ${featureCycles}/${maxFeatureCycles}: ${slice}m slice; model=${env.LOCAL_AI_MODEL}; passes=${featurePassesPerSlice}`);
  const result = spawnSync(process.execPath, ['builder/runner/feature-brain.mjs'], { cwd: root, stdio: 'inherit', env });
  const status = result.error ? 1 : (result.status ?? 1);
  appendAudit('feature-brain-finished', { cycle: featureCycles, status, remainingMinutes: Number(remainingMinutes().toFixed(2)), protocol: 'structured-search-replace-v4' });
  writeRuntimeState(status === 0 ? 'running' : 'running');
  return status;
}
...
while (totalUnits < requestedUnits && remainingMinutes() > 0) {
  ...
  if (status !== 0) {
    ...
    appendAudit('deterministic-recoverable-failure', { iteration, status, objectiveId: state?.objectiveId || null, taskId: state?.blockedTask || null, error: state?.error || null, remainingMinutes: Number(remainingMinutes().toFixed(2)) });
    console.error(`[autobot] deterministic slice failed; preserving working tree and continuing to feature recovery (${deterministicFailures})`);
    writeRuntimeState('running');
  } else {
    ...
    if (state?.status === 'blocked') {
      deterministicFailures++;
      appendAudit('deterministic-recoverable-failure', { iteration, objectiveId: state.objectiveId || null, taskId: state.blockedTask || null, error: state.error || null });
      writeRuntimeState('running');
    }
  }
  ...
  if (consecutiveNoProgress >= maxNoProgressIterations && deterministicFailures === 0) {
...
const summary = { totalUnits, totalObjectives, iterations: iteration, elapsedMinutes: Number(((Date.now() - started) / 60000).toFixed(2)), replenishments, featureBrain: featureCycles > 0, featureCycles, deterministicFailures, consecutiveNoProgress, remainingMinutes: Number(remainingMinutes().toFixed(2)) };
...
console.log(`[autobot] sustained run finished: ${totalUnits}/${requestedUnits} newly verified deterministic units; ${totalObjectives} completed objectives; ${iteration} iterations; ${summary.elapsedMinutes} minutes elapsed; deterministicFailures=${deterministicFailures}; featureCycles=${featureCycles}; auditRecords=${audit.checked}.`);
