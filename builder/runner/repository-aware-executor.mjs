...
function feature(){features++;const slice=Math.max(1,Math.min(10,Math.floor(left())));const script='builder/runner/repository-aware-feature-brain-v2.mjs';const syntax=spawnSync(process.execPath,['--check',script],{cwd:root,encoding:'utf8'});if(syntax.status!==0){console.error('[autobot] stateless feature brain syntax check failed');return 1}return run(script,[],{BUILDER_MAX_MINUTES:String(slice),AUTOBOT_FEATURE_PASSES:String(process.env.AUTOBOT_FEATURE_PASSES_PER_SLICE||2),LOCAL_AI_MODEL:process.env.LOCAL_AI_MODEL||'qwen3:4b-instruct-2507-q4_K_M'})}
fs.mkdirSync(path.join(root,'builder/working'),{recursive:true});index();appendAudit('repository-aware-run-started',{minutes,units,repositoryIndex:true,featureBrain:'v8-stateless'});
