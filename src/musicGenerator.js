...
import { resolvePromptDuration } from './promptDuration.js';
...
async function buildLocalFallback({duration=15,bpm=112}={}){
  const seconds=resolvePromptDuration('',duration);
  ...
  return {audioAvailable:true,audioMimeType:'audio/wav',audioDataUrl,bpm,beatGrid:audioAnalysis?.beatGrid||null,audioAnalysis,durationSeconds:seconds,generationModel:'local-original-safety-fallback',generationMode:'procedural-original'};
}

export async function generateOriginalMusic({prompt='',duration=15,genre,mood,energy,bpm}={}){
  const resolvedDuration=resolvePromptDuration(prompt,duration);
  try{
    const {data}=await requestJson('/api/generate-music',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt,duration:resolvedDuration,genre,mood,energy,bpm}),timeoutMs:120000},{attempts:3,baseDelayMs:900});
    if(!data?.success)throw new Error(data?.error||'Music generator returned an unsuccessful response.');
    if(data?.soundtrack?.audioAvailable&&data?.soundtrack?.audioDataUrl){try{data.soundtrack.audioAnalysis=await analyseAudioDataUrl(data.soundtrack.audioDataUrl,{targetBpm:data.soundtrack.bpm||bpm||120});data.soundtrack.durationSeconds=resolvedDuration;}catch(error){data.soundtrack.audioAnalysis={analysis:'unavailable',warning:error?.message||'Actual audio analysis unavailable.'};}}
    if(data?.soundtrack?.audioAvailable&&data?.soundtrack?.audioDataUrl)return data;
    const fallback=await buildLocalFallback({duration:resolvedDuration,bpm:Number(data?.soundtrack?.bpm)||Number(bpm)||112});
    ...
    const fallback=await buildLocalFallback({duration:resolvedDuration,bpm:Number(bpm)||112});
