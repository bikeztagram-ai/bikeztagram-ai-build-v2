...
export function createOriginalCinematicWav({seconds=15,bpm=112,energy=.78}={}){
 ...
 for(let i=0;i<frames;i++){const t=i/rate,bi=Math.floor(t/beat),hb=Math.floor(t/half),bb=bi%4,barI=Math.floor(t/bar),local=(t%beat)/beat,halfPos=t%half;let s=0;const e=clamp(master*(t<bar*1.5?.6:t<bar*3?.82:t<bar*5?.95:1),.15,1);
