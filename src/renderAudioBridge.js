...
async function attachDecodedAudio(context, audioUrl, stream, options) {
  ...
  if (!arrayBuffer.byteLength) throw new Error('Generated soundtrack is empty.');
  const audioBuffer = await context.decodeAudioData(arrayBuffer.slice(0));
  if (!audioBuffer?.duration) throw new Error('Generated soundtrack decoded with no duration.');
  ...
  if (!track) throw new Error('Decoded soundtrack produced no MediaStream audio track.');
  stream.addTrack(track);
  await context.resume();
  if (context.state !== 'running') throw new Error(`Audio context did not start (state: ${context.state}).`);
  const offset = Math.max(0, number(options?.offset, 0));
  source.start(0, Math.min(offset, Math.max(0, audioBuffer.duration - 0.01)));
...
export async function attachPlanAudioToRenderStream(stream, plan, options = {}) {
  ...
  let decoded = null;
  try {
    if (isDataLike(audioUrl)) decoded = await attachDecodedAudio(context, audioUrl, stream, options);
  } catch (decodedError) {
    console.warn('[AUDIO] Direct generated-audio decode failed; trying media-element fallback.', decodedError);
    try { await context.close(); } catch {}
    return attachPlanAudioToRenderStreamViaElement(stream, renderPlan, options);
  }
  if (decoded) {
    return { stream, plan: renderPlan, enabled: true, beatSync: sync.enabled, audio: null, context, cleanup: () => { decoded.cleanup(); try { context.close(); } catch {} } };
  }
  try { await context.close(); } catch {}
  return attachPlanAudioToRenderStreamViaElement(stream, renderPlan, options);
}
...
async function attachPlanAudioToRenderStreamViaElement(stream, renderPlan, options = {}) {
  ...
  const context = new AudioCtx();
  const audio = new Audio(); audio.crossOrigin = 'anonymous'; audio.src = audioUrl; audio.preload = 'auto'; audio.loop = false;
  try {
    await waitForAudioElement(audio);
    const source = context.createMediaElementSource(audio);
    const destination = context.createMediaStreamDestination();
    const gain = context.createGain(); gain.gain.value = Number.isFinite(Number(options.gain)) ? Number(options.gain) : 1;
    source.connect(gain); gain.connect(destination);
    const track = destination.stream.getAudioTracks()[0];
    if (!track) throw new Error('Generated soundtrack produced no MediaStream audio track.');
    stream.addTrack(track);
    const offset = Math.max(0, number(options.offset, 0));
    if (offset) audio.currentTime = Math.min(offset, Math.max(0, (audio.duration || offset) - .01));
    await context.resume();
    if (context.state !== 'running') throw new Error(`Audio context did not start (state: ${context.state}).`);
    await audio.play();
    ...
    return { stream, plan: renderPlan, enabled: false, beatSync: Boolean(renderPlan?.music?.beatSyncApplied), reason: error?.message || String(error) };
  }
}
