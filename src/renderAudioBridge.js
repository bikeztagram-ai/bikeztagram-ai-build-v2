...
function nearestBeat(time, beats) { if (!beats.length) return time; let best = beats[0]; let distance = Math.abs(time - best.time); for (const beat of beats) { const d = Math.abs(time - best.time); if (d < distance) { best = beat; distance = d; } } return best; }
