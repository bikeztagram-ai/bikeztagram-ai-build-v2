/* BIKEZTAGRAM AI — browser-side QA helpers.
   ...
   This module also installs a passive QA observer because App.jsx already imports it.
   When a newly rendered <video> appears, the browser decodes it, probes playback,
   samples frames for obvious black-output problems, builds an editorial QA report,
   and sends only small diagnostic metadata to /api/qa-report. No video bytes are sent.
*/
...
async function sampleVideoFrames(video, sampleCount = 5) {
  ...
  if (!width || !height || !Number.isFinite(video.duration) || video.duration <= 0) {
    return { samples: [], blackFrameRatio: null, averageLuma: null };
  }
  ...
  if (!ctx) return { samples: [], blackFrameRatio: null, averageLuma: null };
...
export async function validateRenderedVideo(blob, expectedDuration = 15) {
  ...
  try {
    const metadata = await new Promise((resolve, reject) => {
      ...
      video.onerror = () => { clearTimeout(timeout); reject(new Error('QA: browser could not decode the rendered video.')); };
    ...
    const frameQA = await sampleVideoFrames(video, 5);
    ...
    return {
      ...
      verdict: tooDark ? 'FAIL_TOO_DARK' : (durationDifference <= 1.5 ? 'PASS' : 'PASS_WITH_DURATION_DIFFERENCE')
    };
  } finally {
...
export function buildDirectorQAReport({ file, analysis, productionPlan, renderPlan, renderQA }) {
  ...
  const realFootageOnly = generatedScenes.length === 0;
  ...
  const technicalPass = Boolean(renderQA?.passed);
  const editorialPass = editorialChecks.every((item) => item.passed);

  return {
    generatedAt: new Date().toISOString(),
    verdict: technicalPass && editorialPass ? 'PASS' : 'NEEDS_IMPROVEMENT',
    source: file ? { name: file.name, bytes: file.size, type: file.type } : null,
    analysis: analysis ? { filename: analysis.filename || file?.name || '', durationSeconds: Number(analysis.durationInSeconds || 0), bestMomentCount: Array.isArray(analysis.bestMoments) ? analysis.bestMoments.length : 0 } : null,
    director: productionPlan ? { version: productionPlan.version || '', title: productionPlan.title || '', directorSource: productionPlan.directorSource || '', mode: productionPlan.mode || '', targetDuration: Number(productionPlan.targetDuration || 0), plannedDuration: Number(productionPlan.plannedDuration || 0), realSceneCount: realScenes.length, generatedSceneCount: generatedScenes.length, totalSceneCount: scenes.length } : null,
    renderer: { cutCount: cuts.length, sourceTypes: cuts.map((cut) => cut.sourceType || 'uploaded'), motionStyles: cuts.map((cut) => cut.motionStyle || 'static'), transitions: cuts.map((cut) => cut.transition || 'hard-cut') },
    editorialChecks,
    output: renderQA || null,
    notes: ['QA runs in the browser against the actual rendered video.', 'Playback and frame probes confirm the browser can decode, advance and sample the exported video.', 'Only small QA metadata is sent to /api/qa-report; video bytes are never sent there.', 'Blob upload configuration is not part of this QA layer.']
  };
}

export function downloadQAReport(report) {
  const blob = new Blob([JSON.stringify(report, null, 2)], { type: 'application/json' });
  const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = `bikeztagram-qa-${Date.now()}.json`;
  document.body.appendChild(anchor); anchor.click(); anchor.remove(); URL.revokeObjectURL(url);
}

async function postAutoQA(report) {
  try {
    await fetch('/api/qa-report', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(report) });
  } catch (error) { console.warn('[QA] Could not send automatic QA report:', error?.message || error); }
}

function installAutomaticQAObserver() {
  if (typeof window === 'undefined' || typeof MutationObserver === 'undefined') return;
  if (window.__bikeztagramAutoQAInstalled) return;
  window.__bikeztagramAutoQAInstalled = true;
  const tested = new WeakSet();
  const testVideoElement = async (video) => {
    if (!(video instanceof HTMLVideoElement) || tested.has(video)) return;
    tested.add(video);
    ...
    try {
      ...
      clone.muted = true; clone.playsInline = true; clone.preload = 'auto'; clone.src = video.currentSrc || video.src;
      await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => reject(new Error('auto-QA metadata timeout')), 10000);
        clone.onloadedmetadata = () => { clearTimeout(timeout); resolve(); };
        clone.onerror = () => { clearTimeout(timeout); reject(new Error('auto-QA browser decode failed')); };
        clone.load();
      });
      const playbackStart = performance.now(); await clone.play(); await new Promise((resolve) => setTimeout(resolve, 700));
      const advanced = clone.currentTime > 0.05; clone.pause();
      const frameQA = await sampleVideoFrames(clone, 3);
      const report = { generatedAt:new Date().toISOString(), kind:'automatic-render-observer', sourceUrlType:(video.currentSrc || video.src || '').startsWith('blob:') ? 'blob-url' : 'other', durationSeconds:Number(clone.duration.toFixed(2)), width:clone.videoWidth, height:clone.videoHeight, blobLikeOutput:(video.currentSrc || video.src || '').startsWith('blob:'), playbackAdvanced:advanced, playbackProbeMs:Math.round(performance.now()-playbackStart), frameQA, verdict:advanced && !(frameQA.blackFrameRatio != null && frameQA.blackFrameRatio >= 0.67) ? 'PASS' : 'NEEDS_IMPROVEMENT' };
      window.__bikeztagramLastAutoQA = report;
      await postAutoQA(report);
      console.info('[QA] Automatic render QA complete', report);
      clone.pause(); clone.removeAttribute('src'); clone.load();
    } catch (error) {
      const report = { generatedAt:new Date().toISOString(), kind:'automatic-render-observer', verdict:'FAIL', error:error?.message || String(error) };
      window.__bikeztagramLastAutoQA = report; await postAutoQA(report); console.error('[QA] Automatic render QA failed', report);
    }
  };
  const scan = (root) => { if (root instanceof HTMLVideoElement) testVideoElement(root); if (root?.querySelectorAll) root.querySelectorAll('video').forEach(testVideoElement); };
  new MutationObserver((mutations) => mutations.forEach((mutation) => mutation.addedNodes.forEach(scan))).observe(document.documentElement, { childList:true, subtree:true });
  document.querySelectorAll('video').forEach(testVideoElement);
}
