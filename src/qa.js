...
const AUDIO_DURATION_TOLERANCE_SECONDS = 1.5;
...
async function probeRenderedAudio(blobUrl) {
  ...
  } catch (error) { return { detected: false, rms: 0, durationSeconds: null, reason: error?.message || String(error) }; }
...
export async function validateRenderedVideo(blob, expectedDuration = 15, options = {}) {
  ...
  try {
    ...
    const requireAudio = Boolean(options.requireAudio), audioQA = requireAudio ? await probeRenderedAudio(url) : { detected: false, required: false, durationSeconds: null, reason: 'not-required' };
    const audioFailed = requireAudio && !audioQA.detected;
    const audioDurationDifference = requireAudio && Number.isFinite(audioQA.durationSeconds) ? Math.abs(metadata.duration - audioQA.durationSeconds) : null;
    const audioDurationFailed = requireAudio && audioQA.detected && audioDurationDifference != null && audioDurationDifference > AUDIO_DURATION_TOLERANCE_SECONDS;
    return { passed: !tooDark && !audioFailed && !audioDurationFailed, blobBytes: blob.size, blobMB: Number((blob.size / 1024 / 1024).toFixed(2)), durationSeconds: Number(metadata.duration.toFixed(2)), expectedDurationSeconds: Number(expectedDuration || 0), durationDifferenceSeconds: Number(durationDifference.toFixed(2)), width: metadata.width, height: metadata.height, playbackProbeSeconds: Number(playbackSeconds.toFixed(2)), playbackProbeElapsedMs: Math.round(performance.now() - playbackStart), frameQA, audioQA: { ...audioQA, durationDifferenceSeconds: audioDurationDifference == null ? null : Number(audioDurationDifference.toFixed(2)), durationAligned: audioDurationDifference == null ? null : audioDurationDifference <= AUDIO_DURATION_TOLERANCE_SECONDS }, codecCheck: 'Browser decoded rendered video successfully', verdict: tooDark ? 'FAIL_TOO_DARK' : (audioFailed ? 'FAIL_NO_AUDIO' : (audioDurationFailed ? 'FAIL_AUDIO_DURATION' : (durationDifference <= 1.5 ? 'PASS' : 'PASS_WITH_DURATION_DIFFERENCE'))) };
