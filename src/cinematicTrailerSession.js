...
export function updateTrailerShot(session, shotId, patch = {}) { const shots = (session.shots || []).map((shot) => shot.id === shotId ? { ...shot, ...patch } : shot); const failed = shots.some((shot) => shot.status === 'failed'); const complete = shots.length > 0 && shots.every((shot) => shot.status === 'complete'); return updateTrailerSession(session, { shots, status: failed ? 'error' : complete ? 'complete' : 'generating' }); }
export function markShotStarted(session, shotId) { const current = (session.shots || []).find((shot) => shot.id === shotId); return updateTrailerShot(session, shotId, { status: 'generating', attempts: (current?.attempts || 0) + 1, error: null }); }
...
function storageSnapshot(session) {
  return { ...session, results: (session.results || []).map((result) => ({ ...result, blob: undefined })), output: session.output ? { ...session.output, blob: undefined, url: undefined } : null };
}
export function saveTrailerSession(session) { if (!session?.id) throw new Error('Trailer session requires an id.'); localStorage.setItem(STORAGE_PREFIX + session.id, JSON.stringify(storageSnapshot(session))); return session; }
