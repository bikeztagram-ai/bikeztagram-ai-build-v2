...
export async function formatRenderedFilm(blob,{preset='portrait',prompt='',fps=30,onProgress}={}){
  ...
  if(target.id==='portrait')return{blob,preset:target,transcoded:false};
  ...
  const sourceUrl=URL.createObjectURL(blob);video.src=sourceUrl;
  ...
  const ctx=canvas.getContext('2d');if(!ctx)throw new Error('Output canvas is unavailable.');
  const stream=canvas.captureStream(fps),mime=pickMime();if(!mime)throw new Error('No supported browser video encoder is available.');
  const chunks=[],recorder=new MediaRecorder(stream,{mimeType:mime,videoBitsPerSecond:8000000});
  recorder.ondataavailable=(e)=>{if(e.data?.size)chunks.push(e.data);};
  const ended=new Promise((resolve,reject)=>{recorder.onstop=resolve;recorder.onerror=()=>reject(new Error('Output formatting recorder failed.'));});
  ...
  recorder.start(250);await video.play();const timer=setInterval(draw,Math.max(20,Math.round(1000/fps)));draw();await new Promise((resolve)=>{video.onended=resolve;});clearInterval(timer);draw();await new Promise(r=>setTimeout(r,120));recorder.stop();await ended;
  stream.getTracks().forEach(t=>t.stop());video.pause();URL.revokeObjectURL(sourceUrl);onProgress?.(100);
  return{blob:new Blob(chunks,{type:mime}),preset:target,transcoded:true};
}
