...
export default function CinematicStudio() {
  ...
  async function preflightWorker(signal) {
    ...
    const next = { checked: true, configured: Boolean(data?.configured), ready: Boolean(data?.ready), message: data?.message || '' };
    setWorker(next);
    ...
    if (!data?.ready) throw new Error(data?.message || 'Free GPU worker is configured but not currently reachable. No generation job was queued and no reference files were uploaded.');
    return data;
  }
  ...
  return <main style={{ maxWidth: 1000, margin: '0 auto', padding: 32, fontFamily: 'system-ui, sans-serif' }}>
    <header><div style={{ fontSize: 13, opacity: .65, letterSpacing: 1.5 }}>BIKEZTAGRAM AI • CINEMATIC STUDIO</div><h1>Turn your rider + bike references into a trailer.</h1><p style={{ opacity: .75 }}>Direct £0-only generation path. The studio verifies the GPU worker is reachable before uploading references or starting a job.</p></header>
    <section style={{ display: 'grid', gap: 16 }}>
      <div style={{ padding: 12, border: '1px solid #ddd', borderRadius: 10 }}><strong>GPU worker:</strong> {worker.checked ? worker.ready ? 'ready' : worker.configured ? 'configured but unavailable' : 'not configured' : 'not checked'}{worker.message ? ` — ${worker.message}` : ''}</div>
