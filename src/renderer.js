 ...
 *
 * IMPORTANT stability rule: a cut's requested timeline duration is authoritative.
 * If playback reaches the source end early because of speed treatment, freeze the
 * final decoded frame until the cut duration is complete instead of ending the cut.
 */
export async function renderProject(mediaItems, plan, onProgress) {
  ...
  const cuts = Array.isArray(plan?.cuts) ? plan.cuts : Array.isArray(plan?.scenes) ? plan.scenes.map((s, i) => ({
    ...
    directorIntent: s.directorIntent, beatTreatment: s.beatTreatment,
    transition: s.transitionIn || (i ? 'hard-cut' : 'fade-in'), motionStyle: s.motionStyle || 'static',
    motionIntensity: Number(s.motionIntensity) || 0.65, speed: Number(s.speed) || 1,
  ...
  const drawVideo = (video, cut, p) => {
    ...
    const ratio = sw / sh, target = canvas.width / canvas.height, t = transformFor(cut, p);
    let width, height;
    ...
    ctx.save(); ctx.filter = filterFor(cut.colorGrade || plan.colorGrade); ctx.drawImage(video, x, y, width, height); ctx.restore();
  };
  const overlays = (cut, p, index) => {
    ...
    if (/trailer|film|cinematic|commercial/.test(prompt)) {
      const bar = Math.round(canvas.height * .035); ctx.save(); ctx.fillStyle = 'rgba(0,0,0,.9)';
    ...
    if (text) { ctx.save(); ctx.globalAlpha = Math.min(clamp(p/.12,0,1), clamp((1-p)/.12,0,1)); ctx.fillStyle = '#fff'; ctx.font = '800 52px Arial,sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.shadowColor = 'rgba(0,0,0,.95)'; ctx.shadowBlur = 18; ctx.fillText(text.toUpperCase(), canvas.width/2, canvas.height-220); ctx.restore(); }
    if (/action|chase|speed|impact|race|energetic/.test(`${purpose} ${prompt}`) || cut.storyRole === 'escalation') {
      ctx.save(); ctx.globalAlpha = .07 * Math.sin(p*Math.PI); ctx.strokeStyle = '#fff'; ctx.lineWidth = 3;
      for (let i=0;i<5;i++){const y=canvas.height*(.22+i*.12);ctx.beginPath();ctx.moveTo(canvas.width*.12,y);ctx.lineTo(canvas.width*.88,y);ctx.stroke();} ctx.restore();
    }
  };
  const loadVideo = async (video, source) => {
    video.muted = true; video.playsInline = true; video.preload = 'auto'; video.crossOrigin = source.remote ? 'anonymous' : ''; video.src = source.url; video.load();
  ...
  const output = await new Promise((resolve, reject) => {
    recorder.onstop = () => { if (stopped) return; stopped = true; if (!chunks.length) return reject(new Error(`MediaRecorder produced no video data. Codec: ${mimeType || 'browser default'}.`)); resolve(new Blob(chunks, { type: chunks[0]?.type || mimeType || 'video/webm' })); };
    ...
    (async () => {
      try {
        ...
        for (let index = 0; index < cuts.length; index += 1) {
          ...
          const source = sourceFor(media); if (!source) throw new Error(`Cut ${index+1} has no usable source file or Blob URL.`);
          const video = document.createElement('video'); let fallback = null;
          try {
            try { await loadVideo(video, source); }
            catch (error) { if (source.remote && media.file) { fallback = { url: URL.createObjectURL(media.file), revoke: true, remote: false }; await loadVideo(video, fallback); } else throw error; }
            ...
            const requestedSpeedA = clamp(Number(cut.speed) || 1, .5, 1.5), requestedSpeedB = clamp(Number(cut.speedEnd ?? requestedSpeedA) || requestedSpeedA, .5, 1.5);
            ...
            await new Promise((resolveSeek) => { let finished = false; const done = () => { if (finished) return; finished=true; clearTimeout(timer); video.removeEventListener('seeked', done); resolveSeek(); }; const timer = setTimeout(done, 2000); video.addEventListener('seeked', done, { once: true }); });
            video.playbackRate = speedA;
            try { await video.play(); } catch (error) { throw new Error(`Browser refused video playback: ${error?.message || String(error)}`); }
            const started = performance.now(); let sourceEnded = false;
            await new Promise((resolveFrame) => {
              const frame = () => {
                const p = clamp((performance.now()-started)/(duration*1000), 0, 1);
                if (video.ended) { sourceEnded = true; try { video.pause(); } catch {} }
                else { try { video.playbackRate = clamp(speedA + (speedB-speedA)*ease(p), .5, 1.5); } catch {} }
                ctx.fillStyle='#000'; ctx.fillRect(0,0,canvas.width,canvas.height); drawVideo(video, cut, p); overlays(cut, p, index); onProgress?.(Math.round(((index+p)/cuts.length)*100));
                if (p >= 1) return resolveFrame(); requestAnimationFrame(frame);
