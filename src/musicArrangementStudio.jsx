...
export default function MusicArrangementStudio(){
 ...
 return <>
  <button className="arrangement-launch" type="button" onClick={()=>setOpen(v=>!v)} aria-expanded={open}>🎚️ MUSIC LAB</button>
  {open&&<aside className="arrangement-studio" aria-label="Music arrangement studio">
   <div className="arrangement-head"><div><span className="arrangement-kicker">MUSIC LAB</span><h3>Arrange the original score</h3></div><div className="arrangement-head-actions"><strong>{stats.bpm} BPM</strong><button type="button" onClick={()=>setOpen(false)} aria-label="Close Music Lab">×</button></div></div>
