...
function scoreTimeline(cuts){
 ...
 if(motion/cuts.length<.45){score-=8;issues.push('Too many static shots')}
...
function repairTimeline(cuts,flags={}){
 const repaired=cuts.map((input,i)=>{const cut={...input};const role=cut.role||(i===0?'hook':i===cuts.length-1?'hero-ending':'story-beat');cut.role=role;
  // Respect an explicit static instruction. Static is a valid editorial choice,
  // especially for unstable footage, reveals and hero framing.
  ...
  cut.motionIntensity=Number(clamp(n(cut.motionIntensity,.45)+(role==='peak'?.15:0),.25,.6).toFixed(2));
...
export function critiqueAndImproveTimeline(cuts,options={}){if(!Array.isArray(cuts)||!cuts.length)return{cuts:[],before:{score:0,issues:['No shots']},after:{score:0,issues:['No shots']},changed:false};const before=scoreTimeline(cuts);if(before.score>=90)return{cuts,before,after:before,changed:false};const improved=repairTimeline(cuts,options.flags||{});const after=scoreTimeline(improved);return{cuts:improved,before,after,changed:true,improvements:['strengthened story roles','increased purposeful shot variation','tightened pacing','added restrained transition variety','preserved explicit static shots','preserved real source media and subject']}}
