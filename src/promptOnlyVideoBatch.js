...
const buildPrompt = ({ creativePrompt, cut }) => {
  ...
  const role = cut?.purpose || cut?.role || intent.role || 'cinematic shot';
  ...
  const continuity = cut?.creativeIntent?.continuity || 'preserve subject identity, environment, lighting direction and camera language';
...
export async function generatePromptOnlyVideoCutsParallel({ plan, creativePrompt = '', outputPreset = 'portrait', maxGeneratedCuts = 6, concurrency = 3, onProgress } = {}) {
  ...
  const results = await mapWithConcurrency(jobs, Math.max(1, Number(concurrency) || 3), async (cut, index) => {
    ...
    try {
      ...
      const provider = result.provider || 'Runway Gen-4.5';
      const model = result.model || 'gen4.5';
      cut.mediaId = id; cut.generatedMediaId = id; cut.generated = true; cut.mediaIndex = index;
      return { ok: true, index, media: { id, file: result.blob, blob: result.blob, url, sourceUrl: url, mimeType: result.blob.type || 'video/mp4', type: result.blob.type || 'video/mp4', sourceType: 'generated', generated: true, provider, model, generationProvider: result.provider || provider, generationPrompt, sourceRole: cut.role, sourceShotIndex: index } };
    } catch (error) {
      completed += 1;
