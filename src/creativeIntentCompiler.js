...
function lightingFor(brief) {
  const inferred = brief.lighting && brief.lighting !== 'cinematic' ? [brief.lighting] : [];
  return [...inferred, ...(LIGHTING[brief.time] || LIGHTING.day), brief.mood === 'dark' ? 'negative-fill' : 'ambient-fill'].filter((value, index, list) => list.indexOf(value) === index);
}
