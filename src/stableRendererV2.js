/* BIKEZTAGRAM AI — stable renderer v2
 * Sequential playback renderer: seek once per cut, then play normally.
 * Deliberately avoids changing currentTime on every animation frame.
 */
        ...
        for (let index = 0; index < cuts.length; index += 1) {
          ...
          try {
            try { await loadVideo(video, source); }
              ...
              if (source.remote && media.file) { fallback = { url: URL.createObjectURL(media.file), revoke: true, remote: false }; await loadVideo(video, fallback); }
              else throw error;
            }
            ...
            await new Promise((resolveFrame) => {
              const frame = () => {
                ...
                drawVideo(video, cut, p); overlays(cut, p, index);
