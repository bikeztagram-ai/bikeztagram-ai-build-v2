...
export default function App() {
  ...
  function handleFileChange(event) {
    ...
    setRenderedVideoUrl(''); setFillInUrl(''); setProgress(0); setRenderProgress(0); setErrorDetails(null); setCurrentStage(''); setStatus(selected ? `Selected: ${selected.name}` : '');
  }
  ...
  async function createFreeFillIn() {
    ...
    try {
      ...
      if (fillInUrl) URL.revokeObjectURL(fillInUrl); setFillInUrl(URL.createObjectURL(outputBlob)); setRenderProgress(100); setCurrentStage('STEP 7 — Zero-cost AI world scene completed'); setStatus(`✅ Original AI world scene created — ${(outputBlob.size / 1024 / 1024).toFixed(2)} MB. No paid generation service used.`);
  ...
  async function buildAIEdit() {
    ...
    try {
      ...
      if (renderedVideoUrl) URL.revokeObjectURL(renderedVideoUrl); setRenderedVideoUrl(URL.createObjectURL(outputBlob)); setRenderProgress(100); setCurrentStage('STEP 9 — AI edit completed'); setStatus(`✅ AI edit completed successfully — ${(outputBlob.size / 1024 / 1024).toFixed(2)} MB`);
  ...
  function clearVideo() { if (renderedVideoUrl) URL.revokeObjectURL(renderedVideoUrl); if (fillInUrl) URL.revokeObjectURL(fillInUrl); setFile(null); setSourceUrl(''); setAnalysis(null); setPlan(null); setProductionPlan(null); setRenderedVideoUrl(''); setFillInUrl(''); setStatus(''); setProgress(0); setRenderProgress(0); setErrorDetails(null); setCurrentStage(''); }
  ...
  return <div className="app-container"><header className="app-header"><div><h1>BIKEZTAGRAM AI</h1><p>AI-powered motorcycle video editor</p></div></header><main>
    ...
    <div className="button-row">
      ...
      {productionPlan && !loading && !creatingFillIn && !rendering && <button className="generate-btn" onClick={createFreeFillIn}>✨ Create AI World Scene</button>}
    ...
    {(loading || creatingFillIn || rendering) && <section className="status-panel" style={{marginTop:'15px'}}><p className="status-text">{status}</p><p style={{fontSize:'13px',opacity:0.8}}>{currentStage}</p>{loading && <div>Blob upload: {progress}%</div>}{rendering && <div>Render: {renderProgress}%</div>}{creatingFillIn && <div>Browser-only world generation — no paid API.</div>}</section>}
    ...
    {fillInUrl && <section className="result-container" style={{marginTop:'20px'}}><h2>✨ AI World Scene</h2><div className="status-panel"><video src={fillInUrl} controls playsInline autoPlay muted style={{width:'100%',maxWidth:'420px',display:'block',margin:'0 auto',borderRadius:'10px',background:'#000'}}/><p style={{marginTop:'12px'}}>Original procedural environment built inside Bikeztagram from your supplied motorcycle footage. No paid generation service is used.</p></div></section>}
