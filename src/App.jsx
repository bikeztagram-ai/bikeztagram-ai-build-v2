...
export default function App() {
  ...
  async function createFreeWorldScene() {
    ...
    setRenderProgress(0);
    setErrorDetails(null);
    setCurrentStage('STEP 6 — Creating zero-cost AI world scene');
    ...
    try {
      ...
      setCurrentStage('STEP 7 — Zero-cost AI world scene completed');
      setStatus(`✅ Original procedural world scene created — ${(outputBlob.size / 1024 / 1024).toFixed(2)} MB.`);
    } catch (error) {
      const details = makeErrorDetails(error, 'STEP 6 — Creating zero-cost AI world scene');
  ...
  return <div className="app-container"><header className="app-header"><div><h1>BIKEZTAGRAM AI</h1><p>AI-powered motorcycle video editor</p></div></header><main>
    ...
    <div className="button-row">
      <button className="generate-btn" onClick={analyseActualVideo} disabled={busy || !file}>{loading ? '👁️ Analysing Video...' : '👁️ Analyse Actual Video'}</button>
      {productionPlan && !busy && <button className="generate-btn" onClick={createFreeWorldScene}>✨ Create Free AI World Scene</button>}
