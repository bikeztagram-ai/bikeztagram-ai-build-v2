...
export function prepareCreativeContinuity(plan,{creativePrompt='',duration=0}={}){
  ...
  for(let i=0;i<cuts.length;i+=1){
    ...
    // Add a penalty for repeating the same source too frequently
    if (i > 0 && source === cuts[i - 1].mediaIndex) {
      c.repetitionPenalty += 0.5;
    }

    // Add a penalty for shots that are too similar in content or style
    if (i > 0) {
      const previousCut = cuts[i - 1];
      const similarityScore = calculateSimilarity(c, previousCut);
...
function calculateSimilarity(cut1, cut2) {
  // Implement a similarity calculation based on content or style
  ...
  return Math.abs(cut1.duration - cut2.duration) / 8;
}
