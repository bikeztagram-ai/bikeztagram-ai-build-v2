...
export function prepareCreativeContinuity(plan,{creativePrompt='',duration=0}={}){
  ...
  for(let i=0;i<cuts.length;i+=1){
    ...
    c.repetitionPenalty=source!=null&&used.has(source)&&i>0&&cuts[i-1].mediaIndex===source?1:0;
    ...
    // Check for consecutive shots with the same media index
    if (i > 0 && source === cuts[i - 1].mediaIndex) {
      c.repetitionPenalty = 1;
    }
  }
