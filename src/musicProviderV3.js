...
export function createOriginalMusicWav(seconds=15,bpm=112,{energy=.82,seed='bikeztagram-v3'}={}){
 const sr=44100,ch=2,total=c(seconds,5,180),N=Math.floor(total*sr),data=N*4,b=new ArrayBuffer(44+data),v=new DataView(b),u=(o,x)=>v.setUint32(o,x,true),w=(o,x)=>v.setUint16(o,x,true); const s=(o,x)=>[...x].forEach((q,i)=>v.setUint8(o+i,q.charCodeAt(0)));
 ...
 const ev=[];for(let barNo=0;barNo<Math.ceil(total/bar);barNo++){const z=barNo*bar,r=root+prog[barNo%4];for(let q=0;q<16;q++){const t=z+q*beat/4;if(t>=total)break;ev.push({t,d:beat*.22,m:r-12+(q%8===0?0:q%8===4?7:0),g:.13+e*.06,k:0});ev.push({t,d:.035,m:88,g:.018+e*.012,k:2});if(q%2===0)ev.push({t:t+.01,d:beat*.18,m:r+12+scale[(q/2+barNo)%7],g:.025+e*.02,k:2});}for(const off of [0,beat*2])ev.push({t:z+off,d:.16,m:34,g:.42+e*.15,k:0,dr:1});for(const off of [beat,beat*3])if(barNo%2)ev.push({t:z+off,d:.14,m:40,g:.2+e*.08,k:0,sn:1});for(const m of [r,r+scale[2],r+scale[4]])ev.push({t:z,d:Math.min(bar,total-z),m:m+12,g:.035+e*.012,k:0,pad:1});}
