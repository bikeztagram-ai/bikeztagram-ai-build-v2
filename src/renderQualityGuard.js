...
export function validateRenderReadiness({ mediaItems = [], plan = {} } = {}) {
  ...
  if (!Array.isArray(mediaItems) || !mediaItems.length) issues.push('No source media is available.');

  let total = 0;
  cuts.forEach((cut, index) => {
    const duration = Number(cut?.duration);
    const mediaIndex = Number(cut?.mediaIndex);
    if (!Number.isFinite(duration) || duration < 0.5 || duration > 4) issues.push(`Cut ${index + 1} has invalid duration.`);
    if (!Number.isInteger(mediaIndex) || !mediaItems[mediaIndex]) issues.push(`Cut ${index + 1} references missing media.`);
    if (!Number.isFinite(Number(cut?.startTime)) || Number(cut.startTime) < 0) issues.push(`Cut ${index + 1} has invalid source time.`);
  ...
  return { ready: issues.length === 0, issues, totalDuration: Number(total.toFixed(3)), targetDuration: target, cutCount: cuts.length };
}
