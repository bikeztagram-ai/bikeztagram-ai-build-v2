...
export function shapeCinematicEditPlan(plan = {}, moments = []) {
  ...
  for (const cut of cuts) {
    ...
    if (Number.isInteger(index) && !seen.has(index)) { seen.add(index); unique.push(cut); }
    else repeated.push(cut);
  }
  const finalCuts = unique.length >= 3 ? unique.slice(0, 6) : [...unique, ...repeated].slice(0, 6);
  ...
  const hero = [...finalCuts].sort((a, b) => (rankedSet.get(Number(b.momentIndex)) ?? 0) - (rankedSet.get(Number(a.momentIndex)) ?? 0))[0];
  const ordered = [...finalCuts];
  const heroIndex = ordered.indexOf(hero);
  if (heroIndex >= 0 && heroIndex !== ordered.length - 1) { ordered.splice(heroIndex, 1); ordered.push(hero); }
  ...
  return { ...plan, cuts: shaped, editorialStructure: purposes.slice(0, shaped.length) };
}
