...
export function directCreativeRequest({prompt='',assets=[],duration=15,aspectRatio='9:16'}={}){
 ...
 for(const slot of scenePlan.slots){
   if(slot.generation==='preferred') generationRequests.push(buildGenerationRequest({type:'insert',prompt:`${p}. Generate an original cinematic ${slot.role} that bridges real footage and lands precisely on the music event.`,duration:slot.duration,assets,subjectIds:subjects.subjects.map(s=>s.id),timelineSlot:{start:slot.start,role:slot.role},direction:brief.visual}));
 }
 return {version:'creative-direction-v2',brief,media,subjectManifest:subjects,music,scenePlan,generationRequests,decisionLog:[`Parsed natural-language intent: ${p||'cinematic film'}`,`Real media first: ${media.count} assets analysed`,`Music: ${music.genre} at ${music.bpm} BPM`,`Generated scene slots: ${generationRequests.length}`]};
}
