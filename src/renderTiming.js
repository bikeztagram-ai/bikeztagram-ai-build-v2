...
export function buildRenderClock(cuts = []) {
  let elapsed = 0;
  return cuts.map((cut, index) => {
    const duration = Math.max(0.5, Number(cut?.duration) || 2);
    const entry = { index, start: elapsed, end: elapsed + duration, duration };
    elapsed += duration;
    return entry;
  });
}

export function renderCompletionThreshold(cuts = []) {
  const total = cuts.reduce((sum, cut) => sum + Math.max(0.5, Number(cut?.duration) || 2), 0);
  return { expectedSeconds: total, minimumProgress: total > 0 ? 0.995 : 1 };
}
