...
export async function runCreativeEngine({ request, assets = [], targetDuration = 15, director, music, scenes, assemble, render, qa, exportFilm, maxRevisionAttempts = 2 } = {}) {
  ...
  const [musicResult, sceneResult] = await Promise.all([
    music ? music(job.brief, job.assets) : Promise.resolve(null),
    scenes ? scenes(job.brief, job.assets) : Promise.resolve([]),
  ]);
  job = recordGenerationEvidence(job, { kind: 'music', result: musicResult ? 'completed' : 'not-configured' });
  job = recordGenerationEvidence(job, { kind: 'video', result: sceneResult ? 'completed' : 'not-configured' });
  job.outputs.music = musicResult;
  job.outputs.scenes = sceneResult || [];
  job = checkpointCreativeJob(job, 'assemble', { musicReady: Boolean(musicResult), scenesReady: Boolean(sceneResult) });
