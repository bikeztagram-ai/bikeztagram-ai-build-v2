...
function creativeMode(prompt) {
  ...
  return {
    action: /action|chase|aggressive|fast|race|speed|pursuit|fight|battle|adventure|energetic|playful/.test(p),
    ...
    game: /game|open world|gaming/.test(p),
...
function subjectLabel(analysis) {
  return text(analysis?.subject?.label) || text(analysis?.subject?.description) || 'the uploaded media';
}
function purposeFor(moment, index, total, mode) {
  const explicit = text(moment?.editorialRole) || text(moment?.purpose);
  ...
  if (mode.action || isLikelyAction(moment)) return 'action';
  if (mode.reveal || isLikelyReveal(moment)) return 'reveal';
  if (/detail|close-up|macro|face|eyes|object|product/.test(s)) return 'detail';
  if (mode.emotional || /emotion|smile|laugh|sad|beautiful|sunset|landscape|scenery|tender/.test(s)) return 'emotional-beat';
...
function speedFor(purpose, moment, mode) {
  ...
  if (/slow|dramatic|reveal|emotional|beautiful|calm/.test([moment?.description, moment?.reason].join(' ').toLowerCase())) speed = Math.min(speed, 0.78);
...
function gradeFor(analysis, moment, options, mode) {
  ...
  return text(moment?.colorGrade) || text(analysis?.colorGrade) || options.colorGrade || 'cinematic';
}
function sourceDurationFor(moment, analysis) {
  const explicit = num(moment?.sourceDurationInSeconds, 0);
  if (explicit > 0) return clamp(explicit, 0.5, 120);
  const mediaDuration = num(moment?.mediaDurationInSeconds, 0);
  if (mediaDuration > 0) return clamp(mediaDuration, 0.5, 120);
  const analysed = num(analysis?.durationInSeconds, 11);
  return clamp(analysed, 3, 60);
}
function makeCut(moment, index, total, analysis, options, mode, forcedStart = null, forcedDuration = null) {
  const sourceDuration = sourceDurationFor(moment, analysis);
  const sourceStart = forcedStart == null ? clamp(num(moment?.start ?? moment?.startTime, 0), 0, Math.max(0, sourceDuration - 0.5)) : clamp(forcedStart, 0, Math.max(0, sourceDuration - 0.5));
  const sourceEnd = Math.max(sourceStart + 0.5, num(moment?.end ?? moment?.endTime, sourceStart + 3));
  const available = Math.max(0.5, Math.min(sourceDuration - sourceStart, sourceEnd - sourceStart));
  ...
  const duration = forcedDuration == null ? clamp(Math.min(requested, available), 0.5, Math.min(6, options.targetDuration)) : clamp(requested, 0.5, Math.min(6, options.targetDuration));
  ...
  const directedMotion = text(moment?.motionStyle);
  const directedTransition = text(moment?.transition);
  const motionStyle = directedMotion || motionFor(moment, purpose, index, mode);
  const transition = directedTransition || transitionFor(index, total, mode);
  const motionIntensity = num(moment?.motionIntensity, purpose === 'action' ? 1.1 : purpose === 'hero-ending' ? 0.75 : 0.9);
  const focalFraming = moment?.focalFraming || buildFocalFraming(analysis, { ...moment, role: purpose, motionStyle });
  const sourceIndex = Number.isInteger(Number(moment?.sourceIndex)) ? Number(moment.sourceIndex) : Number.isInteger(Number(moment?.mediaIndex)) ? Number(moment.mediaIndex) : 0;
  const selectionScore = num(moment?.directorSelectionScore, 0);
  return {
    mediaIndex: sourceIndex,
    sourceIndex,
    mediaId: text(moment?.mediaId) || undefined,
    sourceDurationInSeconds: Number(sourceDuration.toFixed(2)),
    ...
    transition,
    motionStyle,
    motionIntensity: clamp(motionIntensity, 0, 1.5),
    stabilization: true,
    ...
    textOut: 0.88,
    textStyle: 'cinematic',
    subjectRole: text(moment?.subjectRole) || 'primary-subject',
    directorSelectionScore: selectionScore,
    cameraIntent: text(moment?.cameraIntent) || text(moment?.shotDirection?.cameraIntent),
    shotDirection: moment?.shotDirection,
    focalFraming
  };
}
function storyFallback(analysis, options, mode) {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const fallback = {
      start: index * segment,
      end: Math.min(duration, (index + 1) * segment),
      mediaIndex: num(moment?.mediaIndex, 0),
      sourceIndex: num(moment?.sourceIndex, num(moment?.mediaIndex, 0)),
      mediaId: text(moment?.mediaId),
      sourceDurationInSeconds: num(moment?.sourceDurationInSeconds, sourceDuration)
    };
    const cutMoment = Object.keys(moment).length ? { ...fallback, ...moment } : fallback;
    return makeCut(cutMoment, index, count, analysis, options, mode, fallback.start, segment);
  });
}
function ensureUsefulDuration(cuts, analysis, options, mode) {
  ...
  const supplemented = [];
  for (let i = 0; i < needed; i += 1) supplemented.push(cuts[i] || fallback[i]);
  const total = supplemented.reduce((sum, cut) => sum + num(cut.duration, 0), 0);
  return total < target * 0.7 ? fallback : supplemented;
}
function directorSelectionFromCuts(cuts) {
  return cuts.map((cut, index) => ({
    sourceIndex: Number.isInteger(Number(cut?.sourceIndex)) ? Number(cut.sourceIndex) : index,
    mediaIndex: Number.isInteger(Number(cut?.mediaIndex)) ? Number(cut.mediaIndex) : index,
    mediaId: text(cut?.mediaId) || undefined,
    score: num(cut?.directorSelectionScore, 0),
    editorialRole: text(cut?.editorialRole) || text(cut?.role) || text(cut?.purpose) || 'story-beat',
    cameraIntent: text(cut?.cameraIntent) || undefined,
    motionStyle: text(cut?.motionStyle) || undefined
  }));
}
export function createAIEditPlan(rawAnalysis, options = {}) {
  const analysis = normalizeUniversalAnalysis(rawAnalysis);
  const targetDuration = resolveCreativeDuration(options.creativePrompt, num(options.targetDuration, 15));
  const mode = creativeMode(options.creativePrompt);
  const rawMoments = momentsFrom(analysis);
  const selectedMoments = selectDirectorMoments(rawMoments, { maxCuts: clamp(num(options.maxCuts, 8), 1, 30), targetDuration, creativePrompt: options.creativePrompt });
  ...
  if (analysis?.aiEditPlan?.cuts?.length) {
    ...
    cuts = directed.map((directedCut, index) => {
      ...
      const baseMoment = rawMoments[sourceIndex] || selectedMoments[index] || {};
      const moment = { ...baseMoment, ...directedCut, sourceIndex: baseMoment?.sourceIndex ?? directedCut.sourceIndex, mediaIndex: baseMoment?.mediaIndex ?? directedCut.mediaIndex, mediaId: baseMoment?.mediaId ?? directedCut.mediaId, directorSelectionScore: num(baseMoment?.directorSelectionScore, num(directedCut?.directorSelectionScore, 0)), sourceDurationInSeconds: num(baseMoment?.sourceDurationInSeconds, num(directedCut?.sourceDurationInSeconds, 0)) };
      return makeCut(moment, index, directed.length, analysis, { ...options, targetDuration }, mode);
    });
  } else if (storyBeats.length) {
    cuts = storyBeats.map((moment, index) => makeCut(moment, index, storyBeats.length, analysis, { ...options, targetDuration }, mode));
  } else if (selectedMoments.length) {
    cuts = selectedMoments.map((moment, index) => makeCut(moment, index, selectedMoments.length, analysis, { ...options, targetDuration }, mode));
  } else cuts = storyFallback(analysis, { ...options, targetDuration }, mode);
  if (cuts.length < 3 || cuts.reduce((sum, cut) => sum + num(cut.duration, 0), 0) < Math.min(targetDuration, 7)) cuts = ensureUsefulDuration(cuts, analysis, { ...options, targetDuration }, mode);
  ...
  const critique = critiqueAndImproveTimeline(cuts, { flags: mode });
  cuts = critique.cuts;
  const duration = cuts.reduce((sum, cut) => sum + num(cut.duration, 0), 0);
  const rhythm = analyseDirectorRhythm(cuts);
  const subject = subjectLabel(analysis);
  const style = mode.action ? 'cinematic action edit' : mode.horror ? 'dark cinematic suspense' : mode.game ? 'original interactive-world cinematic' : mode.funny ? 'fast comedic social edit' : mode.emotional ? 'emotional cinematic story' : mode.reveal ? 'cinematic reveal edit' : mode.trailer ? 'cinematic trailer' : 'cinematic creative edit';
  const qualityScore = Math.round(critique.after.score * 0.75 + rhythm.score * 0.25);
  const draft = { cuts, targetDuration, creativePrompt: text(options.creativePrompt) };
  const cinematicQuality = evaluateCinematicOutput(draft, { duration });
  return {
    title: `${subject} — AI Cinematic Edit`,
    style,
    creativePrompt: text(options.creativePrompt),
    subject,
    subjectCategory: text(analysis?.subject?.category) || 'unknown',
...
export function describeAIEditPlan(plan) {
  ...
  return `AI edit plan: ${plan.cuts.length} cuts • Total duration: ${duration.toFixed(1)}s • ${motion} cinematic motion shots • ${transitions} styled transitions • Quality: ${plan.qualityScore ?? '—'}/100 • Rhythm: ${plan.rhythmQuality?.score ?? '—'}/100 • Cinematic QA: ${plan.cinematicQuality?.score ?? '—'}/100 • Source: ${plan.source || 'AI'}`;
}
