...
function buildCutsFromFinalDirector(analysis, options) {
  ...
  for (let index = 0; index < directed.cuts.length && cuts.length < maxCuts; index++) {
    ...
    cuts.push({ mediaIndex: 0, mediaId: 'video-0', startTime, duration, purpose, speed: clamp(numberOr(directedCut.speed, numberOr(source?.speed, 1)), 0.5, 1.5), transition: normaliseTransition(directedCut.transition || source?.transition, index, directed.cuts.length), motionStyle: String(directedCut.motionStyle || source?.motionStyle || 'slow-push'), motionIntensity: purpose === 'hero-ending' ? 0.75 : 0.9, stabilization: true, colorGrade: String(source?.colorGrade || analysis?.colorGrade || options.colorGrade || 'dark-cinematic'), text: String(directedCut.text || source?.text || ''), textIn: index === 0 ? 0.10 : 0.14, textOut: index === 0 ? 0.82 : 0.88, textStyle: 'cinematic' });
  }
...
function buildCutsFromMoments(analysis, options) {
  ...
  for (let index = 0; index < moments.length && cuts.length < maxCuts; index++) {
    ...
    const transition = normaliseTransition(firstDefined(moment.transition, transitionRecommendation), index, moments.length);
    ...
    cuts.push({ mediaIndex: 0, mediaId: 'video-0', startTime: start, duration, purpose, speed, transition, motionStyle: motion, motionIntensity: purpose === 'hero-ending' ? 0.75 : 0.9, stabilization: true, colorGrade: firstDefined(moment.colorGrade, analysis?.colorGrade, options.colorGrade, 'dark-cinematic'), text, textIn: index === 0 ? 0.10 : 0.14, textOut: index === 0 ? 0.82 : 0.88, textStyle: 'cinematic' });
...
export function describeAIEditPlan(plan) {
  ...
  return `AI edit plan: ${plan.cuts.length} cuts • Total duration: ${duration.toFixed(1)}s • ${motionCount} cinematic motion shots • Source: ${plan.source || 'AI'}`;
}
