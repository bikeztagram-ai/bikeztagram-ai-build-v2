...
export function revisePlanAfterQA(plan, qa) {
  ...
  return reasons.length ? { plan: { ...plan, cuts: revisedCuts, qaRevision: { version: 'render-qa-revision-v3', reasons, pass: 1 } }, changed: true, reasons } : { plan, changed: false, reasons: [] };
}
export async function renderInspectImprove({ mediaItems, plan, expectedDuration, onProgress, maxAttempts = 2 } = {}) {
  ...
  for (let attempt = 1; attempt <= limit; attempt += 1) {
    ...
    attempts.push({ attempt, bytes: output.size, qa, audioExpected: Boolean(musicUrl), audioAttached });
