...
/** Convert director coverage decisions into the stable renderer contract. */
export function buildExecutableDirectorPlan({mediaItems=[],decisions=[],targetDuration=15,creativePrompt='',colorGrade='dark-cinematic'}={}){
  ...
  for(let i=0;i<ROLES.length;i++){
    ...
    selected.push({mediaIndex:sourceIndex,mediaId:items[sourceIndex]?.id||`source-${sourceIndex}`,sourceType:'uploaded',generated:false,startTime:Math.max(0,number(candidate.decision.startTime,0)),duration,purpose:role,role,transitionIn:transition,transition,motionStyle:candidate.decision.motionStyle||direction.motion.type,motionIntensity:clamp(number(candidate.decision.motionIntensity,1),.35,1.6),cameraIntent:direction.cameraIntent,subjectType:subject,colorGrade:candidate.decision.colorGrade||colorGrade,stabilization:candidate.decision.stabilization!==false,score:clamp(number(candidate.decision.score,0),0,100),directorDecisionId:candidate.decision.id||`director-${role}-${sourceIndex}`});
  }
  ...
  return {version:'executable-director-v1',title:'Universal AI Film',style:'cinematic',creativePrompt,colorGrade,targetDuration:budget,cuts:selected,director:{coverageRoles:selected.map(c=>c.role),subjectCounts,decisionCount:selected.length,executionReady:true}};
}

export function validateExecutableDirectorPlan(plan,mediaItems=[]){
  const cuts=Array.isArray(plan?.cuts)?plan.cuts:[];const errors=[];const seen=new Set();
