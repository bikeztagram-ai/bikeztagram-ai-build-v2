...
function baseDuration(c, intentValue, i, total) {
  ...
  let d = r === 'hook' ? 0.9 : r === 'hero' ? 1.7 : r === 'action' ? 1.0 : r === 'reveal' ? 1.2 : r === 'emotional' ? 1.7 : 1.3;
  if (quality > 180) d += 0.15;
  if (intentValue.fast) d -= 0.15;
  if (intentValue.calm) d += 0.25;
  if (intentValue.trailer && (r === 'hook' || r === 'reveal')) d -= 0.05;
  // Adjust transition density based on editorial cadence
  if (intentValue.fast) d -= 0.1;
  if (intentValue.calm) d += 0.1;
  return clamp(d, 0.65, 2.4);
}
