...
function normalise(plan, analysis, creativeRequest, targetDuration) {
  ...
  const scenes = plan.scenes.slice(0, 10).map((raw, index) => {
    const sourceType = raw?.sourceType === 'generated' ? 'generated' : 'uploaded';
    ...
    return {
      ...
      continuityNotes: text(raw?.continuityNotes) || 'Preserve the supplied motorcycle as the identity anchor.',
  ...
  return {
    version:'9.0-gemini-two-stage', title:text(plan.title) || 'Bikeztagram AI Director', creativeRequest,
    ...
    directorSource:'gemini-stage-2', generationPolicy:{ paidVideoGeneration:false, externalVideoGenerator:false, rule:'Generated material must be original and must not reproduce named copyrighted characters, vehicles, logos, scenes or soundtracks.' }
  };
}
...
Now design the actual edit. Select the strongest real moments, do not simply use upload order, avoid weak/repetitive footage, and build hook → build → escalation/action → hero ending where supported. Use exact timestamps from verified bestMoments. Use purposeful transitions, varied cinematic motion and useful speed changes. Generated scenes are optional and must be original; never copy a named game, film, TV show, character, logo, vehicle or soundtrack. Keep the motorcycle recognisable and consistent.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('[DIRECTOR] Stage 2 request received.', {filename:analysis?.filename,duration:analysis?.durationInSeconds,bestMoments:Array.isArray(analysis?.bestMoments)?analysis.bestMoments.length:0});
    ...
    console.log('[DIRECTOR] Stage 2 plan created.', {scenes:productionPlan.scenes.length,plannedDuration:productionPlan.plannedDuration});
