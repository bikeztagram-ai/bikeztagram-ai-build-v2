...
{
  "filename":"${filename}","durationSeconds":0,"mediaType":"image",
  ...
  "verifiedEvents":[],"action":"","narrative":{"tone":"","emotion":"","storyPotential":""},
  "visualQuality":{"composition":"","lighting":"","colour":"","sharpness":"","subjectVisibility":"","cinematicPotential":""},
  "cinematicScore":0,"bestMoments":[{"start":0,"end":2,"description":"","reason":"","editorialRole":"","subject":"","shotType":"still-image","score":0}],
  ...
  "textRecommendation":{"useText":false,"text":"","reason":""},"transitionRecommendation":"","motionRecommendation":"","continuityNotes":"","avoid":"","editorialNotes":""
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await ai.models.generateContent({ model:'gemini-3.6-flash', contents:createUserContent([createPartFromUri(imageFile.uri, imageFile.mimeType || contentType), promptForImage(prompt, filename)]), config:{ responseMimeType:'application/json' } });
    ...
    let analysis; try { analysis = JSON.parse(modelText); } catch { throw new Error('Gemini returned invalid image analysis JSON.'); }
    analysis.filename = filename; analysis.mediaType = 'image'; analysis.durationSeconds = 0;
    analysis.source = { type:'uploaded-image', pathname:text(pathname), mimeType:contentType };
    analysis.directorPipeline = { stages:['actual-media-analysis','verified-edit-direction'], stage1:'gemini-3.6-flash-image-analysis', sourceOfTruth:'uploaded-media', architecture:'universal-ai-filmmaker' };
