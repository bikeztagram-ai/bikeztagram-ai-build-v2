...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Source media lives in the dedicated public Blob store. The existing
    // private BLOB_* store remains untouched.
    ...
    // Public Blob objects do not need a signed GET URL. The signed GET URL
    // generated by presignUrl can return 404 for a public store because the
    // object itself is intentionally addressable by its stable public URL.
    // Derive that stable URL from the same public Blob origin used by PUT.
    const publicUrl = new URL(`/${pathname}`, new URL(presignedUrl).origin).toString();
