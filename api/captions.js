...
async function readPrivateBlob(pathname){if(!pathname)throw new Error('No Blob pathname was supplied for private video read.');const token=text(process.env.BLOB_READ_WRITE_TOKEN);if(!token)throw new Error('BLOB_READ_WRITE_TOKEN is missing for private Blob read.');const result=await get(pathname,{access:'private',token});if(!result?.stream)throw new Error('Vercel Blob returned no readable stream.');const chunks=[];if(typeof result.stream.getReader==='function'){const reader=result.stream.getReader();try{while(true){const part=await reader.read();if(part.done)break;if(part.value)chunks.push(Buffer.from(part.value));}}finally{reader.releaseLock?.();}}else{for await(const part of result.stream)chunks.push(Buffer.from(part));}const bytes=Buffer.concat(chunks);if(!bytes.length)throw new Error('Private Blob video read returned an empty object.');return{bytes,contentType:text(result?.blob?.contentType)};}
