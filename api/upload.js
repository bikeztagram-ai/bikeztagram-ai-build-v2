...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!body) {
      return res.status(400).json({ error: "Missing Blob upload request body" });
    }

    // Keep the explicit server-side Blob credential in this route.
    // This is intentionally server-only: the admin read/write token is never
    // sent to the browser. handleUpload derives the short-lived client token.
    ...
    return res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : "Failed to generate Blob client token.",
    });
  }
}
